...
void CRunTestSitePart1::OnRunInit()
{
	int nRet_1, nRet_2=0;

	if (st_sync_info.nInit_Flag[THD_TESTSITE_1] != INIT_CLEAR)		return;		//INIT_CLEAR 일때만 초기화 작업을 한다. 초기화가 끝나면 INIT_CLEAR -> INIT_READY가 되기 떄문에...
	...
	switch (m_nInitStep)
	{
		...
		case 100:
			//kwlee 20160905
			nRet_1 = FAS_IO.get_in_bit(st_io_info.i_Tsite1ExistChk, IO_ON);
			...
			{
				m_strAlarmCode.Format(_T("8%d%04d"), IO_OFF, st_io_info.i_Tsite1ExistChk); 
				CTL_Lib.Alarm_Error_Occurrence(9001, dWARNING, m_strAlarmCode);
			}
		...
		case 1100:
			...
			{
				CTL_Lib.Alarm_Error_Occurrence(9010, dWARNING, m_strAlarmCode);
			}
			break;
		...
		case 1300:
			...
			else
			{
				...
				if (m_dwDoorWaitTime[2] > (DWORD) 60000)
				{
					...
					CTL_Lib.Alarm_Error_Occurrence(9020, dWARNING, m_strAlarmCode);
				}
			}
			break;
		...
		case 70000:
			st_sync_info.nInit_Flag[THD_TESTSITE_1] = INIT_READY; 
...
void CRunTestSitePart1::OnRunMove()
{
	...
	clsFunc.OnThreadFunctionStepTrace(14, m_nRunStep);	
	...
	{ 
		...
		case 2100:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9030, dWARNING, m_strAlarmCode);
		...
		case 3100:
			...
			nRet_1 = FAS_IO.get_in_bit(st_io_info.i_ChamberOpenChk[m_nIO_No],	IO_OFF);  
			nRet_2 = FAS_IO.get_in_bit(st_io_info.i_ChamberCloseChk[m_nIO_No],	IO_ON);
			...
			else
			{
				...
				if (m_dwDoorWaitTime[2] > (DWORD) 60000)
				{
					...
					CTL_Lib.Alarm_Error_Occurrence(9040, dWARNING, m_strAlarmCode);
		...
		case 10200:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9050, dWARNING, m_strAlarmCode);
					m_nRunStep = 10100;
				}
			}
			break;
		...
		case 20200:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9060, dWARNING, m_strAlarmCode);
					m_nRunStep = 20100;
				}
			}
			break;
...
void CRunTestSitePart1::OnThreadRun()
{
	
	CTL_Lib.Process_TestChamber_AcMotor(0, THD_TESTSITE_1, 0); //chamber open/close AC Motor 제어하는 루틴	
	...
	switch(st_handler_info.nRunStatus)
	{
		...
		case dSTOP:
			...
			m_dwDoorWaitTime[0] = GetTickCount(); //james 2016.1003 
...
void CRunTestSitePart1::OnCommunication()
{
	COleDateTime	 tCurr;
	...
	if (st_test_site_info[nSite].nTestBdStart == BD_START)
	{
		...
		if (st_basic_info.nModeDevice == WITH_DVC)
		{
			...
			{
				clsInterC1.OnTesterReady(st_test_site_info[nSite]);
				// jtkim 20160930 Log
...
// 				}
				...
				{ 
					...
					clsInterC1.OnModuleInfo(st_test_site_info[nSite]);
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_MODULE_OK)
				{
					...
					clsInterC1.OnTesterStart(st_test_site_info[nSite]);
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_PGM_CHK)
				{
					...
					if (nTime > 60000)
					{
						...
						clsInterC1.OnPgmDownload(st_test_site_info[nSite]);
					}
					else
					{
...
// 							m_nCloseDoor = NO;
// 
// 							clsInterC1.OnChamberDoor(st_test_site_info[m_nTHD_ID], 1);
// 							// jtkim 20160930 Log
...
// 						if (m_nCloseDoor == YES)
// 						{
// 							st_handler_info.m_tlDooropen[0][1] = GetCurrentTime();
// 							st_handler_info.m_tlDooropen[0][2] = st_handler_info.m_tlDooropen[0][1] - st_handler_info.m_tlDooropen[0][0];
// 							if( st_handler_info.m_tlDooropen[0][2] <= 0 ) st_handler_info.m_tlDooropen[0][0] = GetCurrentTime();
// 							if( st_handler_info.m_tlDooropen[0][2] > 1500 )
// 							{
// 								m_nCloseDoor = NO;
// 
// 								clsInterC1.OnChamberDoor(st_test_site_info[m_nTHD_ID], 1);
		...
		else//WITHOUT
		{
			...
			if (nTime > st_recipe_info.nAbortTime)
			{
				...
				//	nBin = (rand() % 10);
					...
					{
						st_test_site_info[nSite].st_pcb_info[i].nBin = BD_DATA_RETEST; //st_test_site_info[nSite].st_pcb_info[i].nBin = BD_DATA_GOOD;
						//st_test_site_info[nSite].st_pcb_info[i].nBin = BD_DATA_GOOD;
					}
...
void CRunTestSitePart1::Set_TestSite_UpDown( int nOnOff )
{
	...
	if(nOnOff == IO_OFF) //up
	{
		FAS_IO.set_out_bit( st_io_info.o_Tsite1DnCyl, IO_OFF);
		FAS_IO.set_out_bit( st_io_info.o_Tsite1UpCyl, IO_ON);
	}
	else //if(nOnOff == IO_ON)//down
	{
		FAS_IO.set_out_bit( st_io_info.o_Tsite1UpCyl, IO_OFF);
		FAS_IO.set_out_bit( st_io_info.o_Tsite1DnCyl, IO_ON);
	}

	if (nOnOff == IO_ON)
	{
		//LogFunction(_T("TEST_SITE"),_T("UP"),_T("TEST_SITE_1"),_T("CYLINDER"),1,strLogKey,strLogData);
		//2017.0109
		clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),0,_T("TEST_SITE_1"),_T("CYLINDER"),1,strLogKey,strLogData);
	}
	else
	{
		//LogFunction(_T("TEST_SITE"),_T("DOWN"),_T("TEST_SITE_1"),_T("CYLINDER"),1,strLogKey,strLogData);
		//2017.0109
		clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),0,_T("TEST_SITE_1"),_T("CYLINDER"),1,strLogKey,strLogData);
	}
}


int	CRunTestSitePart1::Chk_TestSite_UpDown( int nOnOff )
{
	CString strLogKey[10];
	CString	strLogData[10];
	...
	if (nOnOff == IO_OFF)//UP
	{
		if (m_bTestSiteUpDownFlag == false &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite1UpChk, IO_ON)	== IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite1DnChk, IO_OFF)	== IO_OFF)
		{
				 ...
				 FAS_IO.get_in_bit(st_io_info.i_Tsite1DnChk, IO_OFF)	== IO_OFF)
		{
			...
			{
				//LogFunction(_T("TEST_SITE"),_T("UP"),_T("TEST_SITE_1"),_T("CYLINDER"),1,strLogKey,strLogData); //kwlee 2016.0812
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),1,_T("TEST_SITE_1"),_T("CYLINDER"),1,strLogKey,strLogData);
				return RET_GOOD;
			}
		}
		else
		{
			...
			{
				m_strAlarmCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_Tsite1UpChk); 
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),1,_T("TEST_SITE_1"),_T("CYLINDER"),1,strLogKey,strLogData);
				return RET_ERROR;
			}
	...
	{
		if (m_bTestSiteUpDownFlag == false &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite1UpChk, IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite1DnChk, IO_ON)	== IO_ON)
		{
			m_bTestSiteUpDownFlag		= true;
			m_dwWaitTestSiteUpDown[0]	= GetCurrentTime();
		}
			...
			FAS_IO.get_in_bit(st_io_info.i_Tsite1DnChk, IO_ON)	== IO_ON)
		{
			...
			if (m_dwWaitTestSiteUpDown[2] > (DWORD)st_wait_info.nOnWaitTime[nWaitTime])
			{
				//LogFunction(_T("TEST_SITE"),_T("DOWN"),_T("TEST_SITE_1"),_T("CYLINDER"),1,strLogKey,strLogData); //kwlee 2016.0812
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),1,_T("TEST_SITE_1"),_T("CYLINDER"),1,strLogKey,strLogData);
				return RET_GOOD;
			}
		}
		else
		{
			...
			if (m_dwWaitTestSiteUpDown[2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				m_strAlarmCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_Tsite1DnChk); 
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),1,_T("TEST_SITE_1"),_T("CYLINDER"),1,strLogKey,strLogData);
				return RET_ERROR;
			}
...
void CRunTestSitePart1::OnRunDoor()
{
	int nRet_1, nRet_2;
	int nIO;
	...
	nIO = FAS_IO.get_in_bit(st_io_info.i_TsiteDoorChk[0], IO_OFF);
	...
	case 10150:
		...
		else
		{
			...
			if (m_dwDoorWaitTime[2] > (DWORD) 60000)
			{
				strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_TsiteDoorChk[0]);
				CTL_Lib.Alarm_Error_Occurrence(9070, dWARNING, strAlarmCode);
	...
	case 20150:
		...
		else
		{
			...
			{
				strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_TsiteDoorChk[0]);
				CTL_Lib.Alarm_Error_Occurrence(9080, dWARNING, strAlarmCode);
