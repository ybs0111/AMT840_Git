...
CRunTestSitePart5 clsTestSite5;

CRunTestSitePart5::CRunTestSitePart5(void)
{
	m_nBdNum = 0;
	m_nTHD_ID = THD_TESTSITE_5;
	m_nIO_No  = 4;
	m_nRunStep = -1;
...
void CRunTestSitePart5::OnRunInit()
{
	int nRet_1, nRet_2=0;

	if (st_sync_info.nInit_Flag[THD_TESTSITE_5] != INIT_CLEAR)		return;		//INIT_CLEAR 일때만 초기화 작업을 한다. 초기화가 끝나면 INIT_CLEAR -> INIT_READY가 되기 떄문에...
	...
	switch (m_nInitStep)
	{
		...
		case 100:
			//kwlee 20160905
			//kwlee 2016.1027 임시 막아 놓음..센서 체크 안됨
// 			nRet_1 = FAS_IO.get_in_bit(st_io_info.i_Tsite5ExistChk, IO_ON);
// 			
// 			if (nRet_1 == IO_ON)
// 			{
// 				m_nInitStep = 1000;
...
// 			{
// 				m_strAlarmCode.Format(_T("8%d%04d"), IO_OFF, st_io_info.i_Tsite5ExistChk); 
// 				CTL_Lib.Alarm_Error_Occurrence(9401, dWARNING, m_strAlarmCode);
// 			}
			m_nInitStep = 1000;
			break;
		...
		case 1100:
			...
			{
				CTL_Lib.Alarm_Error_Occurrence(9410, dWARNING, m_strAlarmCode);
			}
			break;
		...
		case 1300:
			...
			else
			{
				...
				if (m_dwDoorWaitTime[2] > (DWORD) 60000)
				{
					...
					CTL_Lib.Alarm_Error_Occurrence(9420, dWARNING, m_strAlarmCode);
				}
			}
			break;
		...
		case 70000:
			st_sync_info.nInit_Flag[THD_TESTSITE_5] = INIT_READY; 
...
void CRunTestSitePart5::OnRunMove()
{
	...
	clsFunc.OnThreadFunctionStepTrace(18, m_nRunStep);	
	...
	{ 
		...
		case 2100:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9430, dWARNING, m_strAlarmCode);
		...
		case 3100:
			...
			else
			{
				...
				if (m_dwDoorWaitTime[2] > (DWORD) 60000)
				{
					...
					CTL_Lib.Alarm_Error_Occurrence(9440, dWARNING, m_strAlarmCode);
		...
		case 10200:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9450, dWARNING, m_strAlarmCode);
		...
		case 20200:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9460, dWARNING, m_strAlarmCode);
...
void CRunTestSitePart5::OnThreadRun()
{
//	return;
	CTL_Lib.Process_TestChamber_AcMotor(0, THD_TESTSITE_5, 4); //chamber open/close AC Motor 제어하는 루틴	
...
void CRunTestSitePart5::OnCommunication()
{
	COleDateTime	 tCurr;
	...
	if (st_test_site_info[nSite].nTestBdStart == BD_START)
	{
		...
		if (st_basic_info.nModeDevice == WITH_DVC)
		{
			...
			{
				clsInterC5.OnTesterReady(st_test_site_info[nSite]);
...
// 				}
				...
				{ 
					...
					clsInterC5.OnModuleInfo(st_test_site_info[nSite]);
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_MODULE_OK)
				{
					...
					clsInterC5.OnTesterStart(st_test_site_info[nSite]);
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_PGM_CHK)
				{
					...
					if (nTime > 60000)
					{
						...
						clsInterC5.OnPgmDownload(st_test_site_info[nSite]);
					}
					else
					{
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_BIN_CHK)
				{
					...
					{
						clsInterC5.OnTesterAbort(st_test_site_info[nSite]);
...
// 							m_nCloseDoor = NO;
// 
// 							clsInterC5.OnChamberDoor(st_test_site_info[m_nTHD_ID], 1);
// 
// 							// jtkim 20160930 Log
...
// 						if (m_nCloseDoor == YES)
// 						{
// 							st_handler_info.m_tlDooropen[4][1] = GetCurrentTime();
// 							st_handler_info.m_tlDooropen[4][2] = st_handler_info.m_tlDooropen[4][1] - st_handler_info.m_tlDooropen[4][0];
// 							if( st_handler_info.m_tlDooropen[4][2] <= 0 ) st_handler_info.m_tlDooropen[4][0] = GetCurrentTime();
// 							if( st_handler_info.m_tlDooropen[4][2] > 1500 )
// 							{
// 								m_nCloseDoor = NO;
// 
// 								clsInterC5.OnChamberDoor(st_test_site_info[m_nTHD_ID], 1);
...
// 						}
				...
				else
				{
					...
					{
						st_test_site_info[nSite].nTestBdStart = BD_DATA_ABORT_CHK;
		...
		else
		{
			...
			if (nTime > st_recipe_info.nAbortTime)
			{
				...
				for (i=0; i<8; i++)
				{
					...
					nBin = (rand() % 10);
					if (nBin == 0 || nBin == 1 || nBin == 2 || nBin == 3 || nBin == 4)
					{
						st_test_site_info[nSite].st_pcb_info[i].nRetestCnt++;
						if (st_test_site_info[nSite].st_pcb_info[i].nRetestCnt > st_recipe_info.nTestRetest_Count)
						{
					...
					{
						st_test_site_info[nSite].st_pcb_info[i].nBin = BD_DATA_GOOD;
						//st_test_site_info[nSite].st_pcb_info[i].nBin =BD_DATA_RETEST; //st_test_site_info[nSite].st_pcb_info[i].nBin = BD_DATA_RETEST;
					}
					else
					{
						st_test_site_info[nSite].st_pcb_info[i].nBin = BD_DATA_GOOD; //st_test_site_info[nSite].st_pcb_info[i].nBin = BD_DATA_GOOD;
					}
...
void CRunTestSitePart5::Set_TestSite_UpDown( int nOnOff )
{
	...
	if(nOnOff == IO_OFF) //up
	{
		FAS_IO.set_out_bit( st_io_info.o_Tsite5DnCyl, IO_OFF);
		FAS_IO.set_out_bit( st_io_info.o_Tsite5UpCyl, IO_ON);
	}
	else //if(nOnOff == IO_ON)//down
	{
		FAS_IO.set_out_bit( st_io_info.o_Tsite5UpCyl, IO_OFF);
		FAS_IO.set_out_bit( st_io_info.o_Tsite5DnCyl, IO_ON);
	}

	if (nOnOff == IO_ON)
	{
		//LogFunction(_T("TEST_SITE"),_T("UP"),_T("TEST_SITE_5"),_T("CYLINDER"),1,strLogKey,strLogData);
		//2017.0109
		clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),1,_T("TEST_SITE_5"),_T("CYLINDER"),1,strLogKey,strLogData);
	}
	else
	{
		//LogFunction(_T("TEST_SITE"),_T("DOWN"),_T("TEST_SITE_5"),_T("CYLINDER"),1,strLogKey,strLogData);
		//2017.0109
		clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),1,_T("TEST_SITE_5"),_T("CYLINDER"),1,strLogKey,strLogData);
	}
}


int	CRunTestSitePart5::Chk_TestSite_UpDown( int nOnOff )
{
	CString strLogKey[10];
	CString	strLogData[10];
	...
	if (nOnOff == IO_OFF)//UP
	{
		if (m_bTestSiteUpDownFlag == false &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite5UpChk, IO_ON)	== IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite5DnChk, IO_OFF)	== IO_OFF)
		{
				 ...
				 FAS_IO.get_in_bit(st_io_info.i_Tsite5DnChk, IO_OFF)	== IO_OFF)
		{
			...
			{
				//LogFunction(_T("TEST_SITE"),_T("UP"),_T("TEST_SITE_5"),_T("CYLINDER"),1,strLogKey,strLogData); //kwlee 2016.0812
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),1,_T("TEST_SITE_5"),_T("CYLINDER"),1,strLogKey,strLogData); 
				return RET_GOOD;
			}
		...
		else
		{
			...
			{
				m_strAlarmCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_Tsite5UpChk); 
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),1,_T("TEST_SITE_5"),_T("CYLINDER"),1,strLogKey,strLogData); 
				return RET_ERROR;
			}
	...
	{
		if (m_bTestSiteUpDownFlag == false &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite5UpChk, IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite5DnChk, IO_ON)	== IO_ON)
		{
			m_bTestSiteUpDownFlag		= true;
			m_dwWaitTestSiteUpDown[0]	= GetCurrentTime();
		}
			...
			FAS_IO.get_in_bit(st_io_info.i_Tsite5DnChk, IO_ON)	== IO_ON)
		{
			...
			if (m_dwWaitTestSiteUpDown[2] > (DWORD)st_wait_info.nOnWaitTime[nWaitTime])
			{
				//LogFunction(_T("TEST_SITE"),_T("DOWN"),_T("TEST_SITE_5"),_T("CYLINDER"),1,strLogKey,strLogData); //kwlee 2016.0812
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),1,_T("TEST_SITE_5"),_T("CYLINDER"),1,strLogKey,strLogData); 
				return RET_GOOD;
			}
		...
		else
		{
			...
			if (m_dwWaitTestSiteUpDown[2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				m_strAlarmCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_Tsite5DnChk); 
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),1,_T("TEST_SITE_5"),_T("CYLINDER"),1,strLogKey,strLogData); 
				return RET_ERROR;
			}
...
void CRunTestSitePart5::OnRunDoor()
{
	int nRet_1, nRet_2;
	int nIO;
	...
	nIO = FAS_IO.get_in_bit(st_io_info.i_TsiteDoorChk[4], IO_OFF);
...
//				clsInterC5.OnChamberDoor(st_test_site_info[m_nTHD_ID], 0);
		...
		case 10150:
			...
			else
			{
				...
				if (m_dwDoorWaitTime[2] > (DWORD) 60000)
				{
					strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_TsiteDoorChk[4]);
					CTL_Lib.Alarm_Error_Occurrence(9470, dWARNING, strAlarmCode);
...
//					clsInterC5.OnChamberDoor(st_test_site_info[m_nTHD_ID], 0);
		...
		case 20150:
			...
			else
			{
				...
				{
					strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_TsiteDoorChk[4]);
					CTL_Lib.Alarm_Error_Occurrence(9480, dWARNING, strAlarmCode);
