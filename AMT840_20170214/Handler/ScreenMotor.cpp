...
void CScreenMotor::OnInitGridMotorPosition(int nPartNum, int nAxisNum)
{
	...
	for(i=0; i<clsExcel.m_nAxisPosNum[nPartNum][nAxisNum]; i++)
	{
		...
		//kwlee 2016.0615
		strName = clsExcel.m_strAxisPosName[nPartNum][nAxisNum][i];
		nLen = strName.GetLength();
		strData = strName.Mid(nLen - 6 , 6);

		if (strData == _T("Offset"))
		{
			m_pGridMotPosition.SetItemBkColour(i + 2, 2, SKY_C, ORANGE_C);
			m_pGridMotPosition.SetItemFont(i + 2, 2, &clsFunc.OnLogFont(20));
			m_pGridMotPosition.SetItemText(i + 2, 2, _T("Apply"));
...
void CScreenMotor::OnOffsetDataApply(int nRow)
{

	CDialog_Message dlgMsg;
	int nResponse;
	CString strData,strPosName;

	dlgMsg.m_nMessageType	= 1;
	dlgMsg.m_strMessage.Format(_T("Do you want to really into this result?"));
	...
	if (nResponse == IDOK)
	{
		if (strPosName == _T("Tray_Offset"))
		{
			st_Offset_info.md_Offset[O_LD_TRAY_OFFSET] = m_dMotorPos[nRow - 2][1];
		}
		else if (strPosName == _T("Buffer_Offset"))
		{
			st_Offset_info.md_Offset[O_LD_BUFF_OFFSET] = m_dMotorPos[nRow - 2][1];
		}
		else if (strPosName == _T("TestSite_Y_Offset"))
		{
			st_Offset_info.md_Offset[O_TEST_Y_OFFSET] = m_dMotorPos[nRow - 2][1];
		}
		else if (strPosName == _T("TestSite_X_Offset"))
		{
			st_Offset_info.md_Offset[O_TEST_X_OFFSET] = m_dMotorPos[nRow - 2][1];
		}
		else if (strPosName == _T("Retest_X_Offset"))
		{
			st_Offset_info.md_Offset[O_RETEST_X_OFFSET] = m_dMotorPos[nRow - 2][1];
		}
	}
}
...
void CScreenMotor::OnSelchangedTreeMotorList(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (NULL == m_treeMotorList.GetParentItem(pNMTreeView->itemNew.hItem))
	{
		for(i=0; i<clsExcel.m_nPartCnt; i++)
		{
			...
			{
				strFileName.Format(_T("c:\\AMT840\\Bmp\\%s"), clsExcel.m_strPartInfo[i]);
...
void CScreenMotor::OnMotorCellClick(NMHDR *pNotifyStruct, LRESULT* pResult)
{
	...
	nLen = strPosName.GetLength();
	strData = strPosName.Mid(nLen - 6,6);
	...
	switch (nCol)
	{
		case 0:
			...
			strFileName.Format(_T("c:\\AMT840\\Bmp\\%s"), strImgName);
			m_imgPosition.Load(strFileName);
...
{
	CWorkManualMove	dlgManualMove;
	int	nResponse;

	nResponse =  (int)dlgManualMove.DoModal();
