...
void CScreenWork::OnInitTree()
{
	...
	bmp.LoadBitmap(IDB_BITMAP_GEAR_1);
	m_pImagelist->Add(&bmp,0xEFEFEF);bmp.DeleteObject();//6
...
void CScreenWork::OnSelchangedTreeView(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	else if (strTmp == _T("Tester NetWork"))
	{
	...
	//kwlee 2016.0913
	else if (strTmp == _T("Vision Teach"))
	{
		...
		if (st_handler_info.cWndTitle != NULL)
		{
			st_handler_info.cWndTitle->PostMessage(WM_STATUS_CHANGE, FORM_NAME, IDW_SCREEN_VISION_TEACH);
		}
		/* ************************************************************************** */
	}
	/////////
...
// 		if (st_handler_info.cWndTitle != NULL)
// 		{
// 			st_handler_info.cWndTitle->PostMessage(WM_STATUS_CHANGE, FORM_NAME, IDW_SCREEN_SET_BARCODE);
// 		}
// 		/* ************************************************************************** */
// 	}
	...
	{
		st_other_info.strPassLevel = _T("T");
