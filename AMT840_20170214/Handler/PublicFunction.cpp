...
int CPublicFunction::Check_LastSiteCurrentSite( CString strLotNo )
{
	int nFuncRet = RET_GOOD;
	int nSite = 0, nEnbSocket = 0;

	if(st_lot_info[LOT_NEXT].strLotNo == "")
	{
		return nFuncRet;
	}

	if( st_lot_info[LOT_CURR].strLotNo != strLotNo )
	{
		return nFuncRet;
	}
	...
	if(st_lot_info[LOT_CURR].nLot_THD_Status[THD_TEST_RBT] >=  LOT_END_START && st_lot_info[LOT_NEXT].nLotStatus >= LOT_START)
	{
		for( int nSiteNum = THD_TESTSITE_1; nSiteNum <= THD_TESTSITE_8; nSiteNum++) 
		{
			...
			{
				nSite++;
				for( int x = 0; x < TEST_SOCKET_PARA; x++)
				{
					if( st_test_site_info[nSiteNum].st_pcb_info[x].nEnable == YES)
					{
						nEnbSocket++;
					}
				}
	...
	if( nSite <= 1 && nEnbSocket <= 5) //마지막 사이트이면 socket off를 하지 않는다
	{
		nFuncRet = RET_ERROR;
	}
	return nFuncRet;
