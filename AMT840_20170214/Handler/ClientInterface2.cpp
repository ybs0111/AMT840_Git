...
CClientInterface2::CClientInterface2(void)
{
	// 공유 메모리 선언...../////////////////////////////////////////////////////////////////////////////////////////
	m_hMemMapInter2	= NULL;
	m_pQueueInter2	= NULL;

	m_hMemMapInter2 = CreateFileMapping(INVALID_HANDLE_VALUE,
										 NULL,
										 PAGE_READWRITE,
										 0,
										 sizeof(INTER_CLIENT_QUEUE_2),
										 (LPCWSTR)INTERFACE_CLIENT_MSG_NAME_2);
	if (m_hMemMapInter2 != NULL)
	{	
		if (GetLastError() != ERROR_ALREADY_EXISTS)
		{	
			m_pQueueInter2 = (P_INTER_CLIENT_QUEUE_2)MapViewOfFile(m_hMemMapInter2, FILE_MAP_ALL_ACCESS, 0, 0, 0);
			if (m_pQueueInter2 != NULL)
			{
				m_pQueueInter2->m_nFront	= 0;
				m_pQueueInter2->m_nRear	= 0;
				m_pQueueInter2->m_nCount	= 0;
			}
		}
	}
...
CClientInterface2::~CClientInterface2(void)
{
	if (m_pQueueInter2 != NULL)	UnmapViewOfFile(m_pQueueInter2);	
	if (m_hMemMapInter2 != NULL)	CloseHandle(m_hMemMapInter2);
}


void CClientInterface2::OnCommunication()
{
	...
	switch (m_nCommStep)
	{
		case 0:
			// 공유메모리에 전송할 데이터가 있으면 
			if (m_pQueueInter2->m_nCount <= 0) break;
		...
		case 2000:
			...
			else
			{
				...
				::SendMessage(st_handler_info.hWnd, WM_CLIENT_MSG + INTERFACE_NETWORK_2, CLIENT_CONNECT, 0);
			...
			else
			{
				...
				if (m_dwWaitTime[2] > 30000)
				{
					...
					if (m_nRetry > 1)
					{
						...
						::SendMessage(st_handler_info.hWnd, WM_CLIENT_MSG + INTERFACE_NETWORK_2, CLIENT_CLOSE, 0);

						m_dwWaitTime[0] = GetTickCount();
						CTL_Lib.Alarm_Error_Occurrence(20000, dWARNING, _T("720000"));
		...
		case 2200:
			m_dwWaitTime[0] = GetTickCount();
			st_client_info[INTERFACE_NETWORK_2].strSend = m_strSendMsg;

			::SendMessage(st_handler_info.hWnd, WM_CLIENT_MSG + INTERFACE_NETWORK_2, CLIENT_SEND, 0);
		...
		case 2300:
			if (m_nCommBin == BD_DATA_GOOD)
			{
				...
				///////////////////////////////////////////////
				//2016.1203
				st_product_info.dTestTime += (double)st_test_site_info[THD_TESTSITE_2].dwBdTime;
			...
			else
			{
				...
				if (m_dwWaitTime[2] > 300000)//최대 3분을 기다리다가 다시 명령어 전송 //2016.1027
				{
					...
					if (m_nRetry > 1)
					{
						...
						m_dwWaitTime[0] = GetTickCount();
						CTL_Lib.Alarm_Error_Occurrence(20001, dWARNING, _T("720001"));
...
CString	CClientInterface2::OnCharToString(char chData[], int nLength)
{
	CString strTmp;
	BSTR	buf;
	int		nCnt;
...
void CClientInterface2::OnStringToChar(CString strData, char chData[])
{
	int nCnt;
	wchar_t *chBuf;
...
void CClientInterface2::OnMessageWrite(CString strMsg)
{
	CString strLog = _T("[HANDLER >> EC SERVER] ");

	if (m_hMemMapInter2 != NULL && m_pQueueInter2 != NULL)
	{
		strLog += strMsg;
		clsFunc.OnLogFileAdd(4, strLog);

		OnStringToChar(strMsg, m_pQueueInter2->m_chMsg[m_pQueueInter2->m_nRear]);

		m_pQueueInter2->m_nRear++;
		
		if (m_pQueueInter2->m_nRear >= 10) m_pQueueInter2->m_nRear = 0;
		
		m_pQueueInter2->m_nCount++;
	}
}

CString	CClientInterface2::OnMessageRead()
{
	CString strData = _T("");
	
	if (m_hMemMapInter2 != NULL && m_pQueueInter2 != NULL)
	{
		if (m_pQueueInter2->m_nCount > 0 )
		{
			strData = m_pQueueInter2->m_chMsg[m_pQueueInter2->m_nFront];
			
			m_pQueueInter2->m_nFront++;
			if (m_pQueueInter2->m_nFront >= 10) m_pQueueInter2->m_nFront = 0;
...
void CClientInterface2::OnDataReceive(CString strMsg)
{
	int nHead;
	int nLength;
	int nPos;
	...
	CString strData;
	CString strFunc;
	CString strLog = _T("[INTERFACE SERVER 2 >> HANDLER] ");
	...
	{
		strTemp.Format(_T("[INTERFACE_CLIENT_2_NETWORK] %s"), strMsg);
		clsMem.OnNormalMessageWrite(strTemp);
	}
}


void CClientInterface2::OnDataAnalysis(CString strMsg)
{
	CString strFunc;
	CString strData;
	CString strTemp;
	...
	else if (strFunc == _T("TEST_START"))
	{
		...
		if (strData == _T("PASS"))
		{
			...
			for (i=0; i<8; i++)
			{
				...
				{
					strLogData[0]	= st_test_site_info[THD_TESTSITE_2].st_pcb_info[i].strSerialNo;
					clsLog.LogProcess(_T("TESTER#2"), 
 								  _T("CDIMM TEST"), 
 								  0, 
 								  st_test_site_info[THD_TESTSITE_2].st_pcb_info[i].strSerialNo, 
 								  st_test_site_info[THD_TESTSITE_2].st_pcb_info[i].strLotNo, 
...
CString CClientInterface2::OnDataItemAnalysis(int nOpt, int nPos, CString strMsg, CString strFind)
{
	CString strData = _T("");
	CString strHead;
	int length;
...
void CClientInterface2::OnTesterReady(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
	...
	strData += strTemp;
	strData += _T(" ");
	//test_site.strOptCode.Format(_T("00")); //2016.1026 Test
...
void CClientInterface2::OnSocketInfo(tagTEST_SITE_INFO &test_site)
{
	int i;
...
void CClientInterface2::OnModuleInfo(tagTEST_SITE_INFO &test_site)
{
	int i;
...
void CClientInterface2::OnTesterStart(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface2::OnPgmDownload(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface2::OnLotComplete(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface2::OnTesterOption(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface2::OnTesterAbort(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface2::OnChamberDoor(tagTEST_SITE_INFO &test_site, int nStatus)
{
	CString strTemp;
	CString strData;
	CString strMsg;
