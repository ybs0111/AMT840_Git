...
void CWorkMotorSpeed::OnDataApply()
{
	...
	{
		//2017.0109
		if (st_motor_info[i].d_accel[0]				!= m_dAcc[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("ACCEL");
			m_strLogKey[1]	= _T("MOTOR");
			m_strLogData[1].Format(_T("%s"), clsLog.GetDeviceID(i));
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%.3f"), st_motor_info[i].d_accel[0]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%.3f"), m_dAcc[i][1]);

			clsLog.LogConfig(_T("WORKSPEED"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_motor_info[i].d_decel[0]				!= m_dDec[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("DECEL");
			m_strLogKey[1]	= _T("MOTOR");
			m_strLogData[1].Format(_T("%s"), clsLog.GetDeviceID(i));
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%.3f"), st_motor_info[i].d_decel[0]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%.3f"), m_dDec[i][1]);

			clsLog.LogConfig(_T("WORKSPEED"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_motor_info[i].d_spd_max[0]				!= m_dMax[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("MAX SPEED");
			m_strLogKey[1]	= _T("MOTOR");
			m_strLogData[1].Format(_T("%s"), clsLog.GetDeviceID(i));
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%.3f"), st_motor_info[i].d_spd_max[0]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%.3f"), m_dMax[i][1]);

			clsLog.LogConfig(_T("WORKSPEED"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_motor_info[i].d_spd_vel[0]				!= m_dVel[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("VEL SPEED");
			m_strLogKey[1]	= _T("MOTOR");
			m_strLogData[1].Format(_T("%s"), clsLog.GetDeviceID(i));
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%.3f"), st_motor_info[i].d_spd_vel[0]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%.3f"), m_dVel[i][1]);

			clsLog.LogConfig(_T("WORKSPEED"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_motor_info[i].d_spd_jog[0]				!= m_dJog[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("JOG SPEED");
			m_strLogKey[1]	= _T("MOTOR");
			m_strLogData[1].Format(_T("%s"), clsLog.GetDeviceID(i));
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%.3f"), st_motor_info[i].d_spd_jog[0]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%.3f"), m_dJog[i][1]);

			clsLog.LogConfig(_T("WORKSPEED"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_motor_info[i].d_spd_home[0]				!= m_dHome[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("HOME SPEED");
			m_strLogKey[1]	= _T("MOTOR");
			m_strLogData[1].Format(_T("%s"), clsLog.GetDeviceID(i));
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%.3f"), st_motor_info[i].d_spd_home[0]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%.3f"), m_dHome[i][1]);

			clsLog.LogConfig(_T("WORKSPEED"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_motor_info[i].d_allow				!= m_dAllow[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("ALLOW");
			m_strLogKey[1]	= _T("MOTOR");
			m_strLogData[1].Format(_T("%s"), clsLog.GetDeviceID(i));
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%.3f"), st_motor_info[i].d_allow);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%.3f"), m_dAllow[i][1]);

			clsLog.LogConfig(_T("WORKSPEED"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_motor_info[i].d_limit_position[0]				!= m_dLimitM[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("M LIMIT");
			m_strLogKey[1]	= _T("MOTOR");
			m_strLogData[1].Format(_T("%s"), clsLog.GetDeviceID(i));
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%.3f"), st_motor_info[i].d_limit_position[0]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%.3f"), m_dLimitM[i][1]);

			clsLog.LogConfig(_T("WORKSPEED"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_motor_info[i].d_limit_position[1]				!= m_dLimitP[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("P LIMIT");
			m_strLogKey[1]	= _T("MOTOR");
			m_strLogData[1].Format(_T("%s"), clsLog.GetDeviceID(i));
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%.3f"), st_motor_info[i].d_limit_position[1]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%.3f"), m_dLimitP[i][1]);
