...
#define INTERFACE_CLIENT_MSG_NAME_5		"INTERFACE_CLIENT_SHAREMEM_5"

CClientInterface5 clsInterC5;

CClientInterface5::CClientInterface5(void)
{
	// 공유 메모리 선언...../////////////////////////////////////////////////////////////////////////////////////////
	...
	m_hMemMapInter5 = CreateFileMapping(INVALID_HANDLE_VALUE,
										 ...
										 PAGE_READWRITE,
										 0,
										 sizeof(INTER_CLIENT_QUEUE_5),
										 (LPCWSTR)INTERFACE_CLIENT_MSG_NAME_5);
	if (m_hMemMapInter5 != NULL)
	{	
		if (GetLastError() != ERROR_ALREADY_EXISTS)
		{	
			m_pQueueInter5 = (P_INTER_CLIENT_QUEUE_5)MapViewOfFile(m_hMemMapInter5, FILE_MAP_ALL_ACCESS, 0, 0, 0);
...
void CClientInterface5::OnCommunication()
{
	...
	switch (m_nCommStep)
	{
		case 0:
			// 공유메모리에 전송할 데이터가 있으면 
			if (m_pQueueInter5->m_nCount <= 0) break;
		...
		case 2000:
			...
			else
			{
				...
				::SendMessage(st_handler_info.hWnd, WM_CLIENT_MSG + INTERFACE_NETWORK_5, CLIENT_CONNECT, 0);
			...
			else
			{
				...
				if (m_dwWaitTime[2] > 30000)
				{
					...
					if (m_nRetry > 1)
					{
						...
						::SendMessage(st_handler_info.hWnd, WM_CLIENT_MSG + INTERFACE_NETWORK_5, CLIENT_CLOSE, 0);

						m_dwWaitTime[0] = GetTickCount();
						CTL_Lib.Alarm_Error_Occurrence(20000, dWARNING, _T("750000"));
		...
		case 2200:
			m_dwWaitTime[0] = GetTickCount();
			st_client_info[INTERFACE_NETWORK_5].strSend = m_strSendMsg;

			::SendMessage(st_handler_info.hWnd, WM_CLIENT_MSG + INTERFACE_NETWORK_5, CLIENT_SEND, 0);
		...
		case 2300:
			if (m_nCommBin == BD_DATA_GOOD)
			{
				...
				///////////////////////////////////////////////
				//2016.1203
				st_product_info.dTestTime += (double)st_test_site_info[THD_TESTSITE_5].dwBdTime;
			...
			else
			{
				...
				if (m_dwWaitTime[2] > 300000)//최대 3분을 기다리다가 다시 명령어 전송 //2016.1027
				{
					...
					if (m_nRetry > 1)
					{
						...
						m_dwWaitTime[0] = GetTickCount();
						CTL_Lib.Alarm_Error_Occurrence(20001, dWARNING, _T("750001"));
...
CString	CClientInterface5::OnCharToString(char chData[], int nLength)
{
	CString strTmp;
	BSTR	buf;
	int		nCnt;
...
void CClientInterface5::OnStringToChar(CString strData, char chData[])
{
	int nCnt;
	wchar_t *chBuf;
...
void CClientInterface5::OnMessageWrite(CString strMsg)
{
...
CString	CClientInterface5::OnMessageRead()
{
	CString strData = _T("");
...
void CClientInterface5::OnDataReceive(CString strMsg)
{
	int nHead;
	int nLength;
	int nPos;
	...
	CString strData;
	CString strFunc;
	CString strLog = _T("[INTERFACE SERVER 5 >> HANDLER] ");
	...
	{
		strTemp.Format(_T("[INTERFACE_CLIENT_5_NETWORK] %s"), strMsg);
		clsMem.OnNormalMessageWrite(strTemp);
	}
}


void CClientInterface5::OnDataAnalysis(CString strMsg)
{
	CString strFunc;
	CString strData;
	CString strTemp;
...
// 				}
		...
		else
		{
			...
			//2016.1101				
			st_handler_info.mstr_event_msg[0].Format(_T("%d site %s,엔지니어 호출 해주세요"), 5, m_strErrorMsg );
	...
	else if (strFunc == _T("TEST_START"))
	{
		...
		if (strData == _T("PASS"))
		{
			...
			for (i=0; i<8; i++)
			{
				...
				{
					strLogData[0]	= st_test_site_info[THD_TESTSITE_5].st_pcb_info[i].strSerialNo;
					clsLog.LogProcess(_T("TESTER#5"), 
 								  _T("CDIMM TEST"), 
 								  0, 
 								  st_test_site_info[THD_TESTSITE_5].st_pcb_info[i].strSerialNo, 
 								  st_test_site_info[THD_TESTSITE_5].st_pcb_info[i].strLotNo, 
...
CString CClientInterface5::OnDataItemAnalysis(int nOpt, int nPos, CString strMsg, CString strFind)
{
	CString strData = _T("");
	CString strHead;
	int length;
...
void CClientInterface5::OnTesterReady(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface5::OnSocketInfo(tagTEST_SITE_INFO &test_site)
{
	int i;
...
void CClientInterface5::OnModuleInfo(tagTEST_SITE_INFO &test_site)
{
	int i;
...
// 	test_site.st_pcb_info[0].nYesNo = YES; //kwlee 2016.0929
// 	test_site.st_pcb_info[1].nYesNo = YES; //kwlee 2016.0929
// 	test_site.st_pcb_info[2].nYesNo = YES; //kwlee 2016.0929
// 	test_site.st_pcb_info[3].nYesNo = YES; //kwlee 2016.0929
// 	test_site.st_pcb_info[4].nYesNo = YES; //kwlee 2016.0929
// 	test_site.st_pcb_info[5].nYesNo = NO; //kwlee 2016.0929
// 	test_site.st_pcb_info[6].nYesNo = YES; //kwlee 2016.0929
// 	test_site.st_pcb_info[7].nYesNo = YES; //kwlee 2016.0929

	//test_site.nModuleCount = 7;
	switch (test_site.nTestMode)
	{ 
		case 0:
			strTemp.Format(_T("TEST_MODE=%s"), _T("ONESHOT"));
	...
	for (i=0; i<8; i++)
	{
		//test_site.st_pcb_info[i].nYesNo = YES; //kwlee 2016.0929
...
void CClientInterface5::OnTesterStart(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface5::OnPgmDownload(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface5::OnLotComplete(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface5::OnTesterOption(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface5::OnTesterAbort(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface5::OnChamberDoor(tagTEST_SITE_INFO &test_site, int nStatus)
{
	CString strTemp;
	CString strData;
	CString strMsg;
