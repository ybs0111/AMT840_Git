...
CRunTestSitePart3 clsTestSite3;

CRunTestSitePart3::CRunTestSitePart3(void)
{
	m_nBdNum = 0;

	m_nTHD_ID = THD_TESTSITE_3;
	m_nIO_No  = 2;
	m_nRunStep = -1;
	// jtkim 20161005
	m_nInitStep = -1;
...
void CRunTestSitePart3::OnRunInit()
{
	int nRet_1, nRet_2=0;

	if (st_sync_info.nInit_Flag[THD_TESTSITE_3] != INIT_CLEAR)		return;		//INIT_CLEAR 일때만 초기화 작업을 한다. 초기화가 끝나면 INIT_CLEAR -> INIT_READY가 되기 떄문에...
	st_sync_info.nInit_Flag[THD_TESTSITE_3] = INIT_READY; 
	...
	switch (m_nInitStep)
	{
		...
		case 100:
			//kwlee 2016.0905
			nRet_1 = FAS_IO.get_in_bit(st_io_info.i_Tsite3ExistChk, IO_ON);
			...
			{
				m_strAlarmCode.Format(_T("8%d%04d"), IO_OFF, st_io_info.i_Tsite3ExistChk); 
				CTL_Lib.Alarm_Error_Occurrence(9210, dWARNING, m_strAlarmCode);
			}
			break;
		...
		case 1100:
			...
			{
				CTL_Lib.Alarm_Error_Occurrence(9220, dWARNING, m_strAlarmCode);
			}
			break;
		...
		case 1300:
			...
			else
			{
				...
				if (m_dwDoorWaitTime[2] > (DWORD) 60000)
				{
					...
					CTL_Lib.Alarm_Error_Occurrence(9230, dWARNING, m_strAlarmCode);
				}
			}
			break;
...
void CRunTestSitePart3::OnRunMove()
{
	...
	clsFunc.OnThreadFunctionStepTrace(16, m_nRunStep);	
	...
	{ 
		...
		case 2100:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9240, dWARNING, m_strAlarmCode);
		...
		case 3100:
			...
			else
			{
				...
				if (m_dwDoorWaitTime[2] > (DWORD) 60000)
				{
					...
					CTL_Lib.Alarm_Error_Occurrence(9250, dWARNING, m_strAlarmCode);
		...
		case 20200:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9270, dWARNING, m_strAlarmCode);
...
void CRunTestSitePart3::OnThreadRun()
{
//	return;
	CTL_Lib.Process_TestChamber_AcMotor(0, THD_TESTSITE_3, 2); //chamber open/close AC Motor 제어하는 루틴	
//	OnCommunication();
	OnCommunication();
...
void CRunTestSitePart3::OnCommunication()
{
	COleDateTime	 tCurr;
	...
	if (st_test_site_info[nSite].nTestBdStart == BD_START)
	{
		...
		if (st_basic_info.nModeDevice == WITH_DVC)
		{
			...
			{
				clsInterC3.OnTesterReady(st_test_site_info[nSite]);
...
// 				}
				...
				{ 
					...
					clsInterC3.OnModuleInfo(st_test_site_info[nSite]);
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_MODULE_OK)
				{
					...
					clsInterC3.OnTesterStart(st_test_site_info[nSite]);
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_PGM_CHK)
				{
					...
					if (nTime > 60000)
					{
						...
						clsInterC3.OnPgmDownload(st_test_site_info[nSite]);
					}
					else
					{
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_BIN_CHK)
				{
					...
					{
						clsInterC3.OnTesterAbort(st_test_site_info[nSite]);
...
// 							m_nCloseDoor = NO;
// 
// 							clsInterC3.OnChamberDoor(st_test_site_info[m_nTHD_ID], 1);
// 
// 							// jtkim 20160930 Log
...
// 						if (m_nCloseDoor == YES)
// 						{
// 							st_handler_info.m_tlDooropen[2][1] = GetCurrentTime();
// 							st_handler_info.m_tlDooropen[2][2] = st_handler_info.m_tlDooropen[2][1] - st_handler_info.m_tlDooropen[2][0];
// 							if( st_handler_info.m_tlDooropen[2][2] <= 0 ) st_handler_info.m_tlDooropen[2][0] = GetCurrentTime();
// 							if( st_handler_info.m_tlDooropen[2][2] > 1500 )
// 							{
// 								m_nCloseDoor = NO;
// 
// 								clsInterC3.OnChamberDoor(st_test_site_info[m_nTHD_ID], 1);
...
void CRunTestSitePart3::Set_TestSite_UpDown( int nOnOff )
{
	...
	if(nOnOff == IO_OFF) //up
	{
		FAS_IO.set_out_bit( st_io_info.o_Tsite3DnCyl, IO_OFF);
		FAS_IO.set_out_bit( st_io_info.o_Tsite3UpCyl, IO_ON);
	}
	else //if(nOnOff == IO_ON)//down
	{
		FAS_IO.set_out_bit( st_io_info.o_Tsite3UpCyl, IO_OFF);
		FAS_IO.set_out_bit( st_io_info.o_Tsite3DnCyl, IO_ON);
	}

	if (nOnOff == IO_ON)
	{
		//LogFunction(_T("TEST_SITE"),_T("UP"),_T("TEST_SITE_3"),_T("CYLINDER"),1,strLogKey,strLogData);
		//2017.0109
		clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),0,_T("TEST_SITE_3"),_T("CYLINDER"),1,strLogKey,strLogData);
	}
	else
	{
		//LogFunction(_T("TEST_SITE"),_T("DOWN"),_T("TEST_SITE_3"),_T("CYLINDER"),1,strLogKey,strLogData);
		//2017.0109
		clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),0,_T("TEST_SITE_3"),_T("CYLINDER"),1,strLogKey,strLogData);
	}
}


int	CRunTestSitePart3::Chk_TestSite_UpDown( int nOnOff )
{
	CString strLogKey[10];
	CString	strLogData[10];
	...
	if (nOnOff == IO_OFF)//UP
	{
		if (m_bTestSiteUpDownFlag == false &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite3UpChk, IO_ON)	== IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite3DnChk, IO_OFF)	== IO_OFF)
		{
				 ...
				 FAS_IO.get_in_bit(st_io_info.i_Tsite3DnChk, IO_OFF)	== IO_OFF)
		{
			...
			{
				//LogFunction(_T("TEST_SITE"),_T("UP"),_T("TEST_SITE_3"),_T("CYLINDER"),1,strLogKey,strLogData); //kwlee 2016.0812
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),1,_T("TEST_SITE_3"),_T("CYLINDER"),1,strLogKey,strLogData); 
				return RET_GOOD;
			}
		}
		else
		{
			...
			{
				m_strAlarmCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_Tsite3UpChk); 
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),1,_T("TEST_SITE_3"),_T("CYLINDER"),1,strLogKey,strLogData); 
				return RET_ERROR;
			}
	...
	{
		if (m_bTestSiteUpDownFlag == false &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite3UpChk, IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite3DnChk, IO_ON)	== IO_ON)
		{
			m_bTestSiteUpDownFlag		= true;
			m_dwWaitTestSiteUpDown[0]	= GetCurrentTime();
		}
			...
			FAS_IO.get_in_bit(st_io_info.i_Tsite3DnChk, IO_ON)	== IO_ON)
		{
			...
			if (m_dwWaitTestSiteUpDown[2] > (DWORD)st_wait_info.nOnWaitTime[nWaitTime])
			{
				//LogFunction(_T("TEST_SITE"),_T("DOWN"),_T("TEST_SITE_3"),_T("CYLINDER"),1,strLogKey,strLogData); //kwlee 2016.0812
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),1,_T("TEST_SITE_3"),_T("CYLINDER"),1,strLogKey,strLogData);
				return RET_GOOD;
			}
		}
		else
		{
			...
			if (m_dwWaitTestSiteUpDown[2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				m_strAlarmCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_Tsite3DnChk); 
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),1,_T("TEST_SITE_3"),_T("CYLINDER"),1,strLogKey,strLogData);
				return RET_ERROR;
			}
...
void CRunTestSitePart3::OnRunDoor()
{
	int nRet_1, nRet_2;
	int nIO;
	...
	nIO = FAS_IO.get_in_bit(st_io_info.i_TsiteDoorChk[2], IO_OFF);
	...
	switch (m_nDoorStep)
	{
		...
		case 10150:
			...
			else
			{
				...
				if (m_dwDoorWaitTime[2] > (DWORD) 60000)
				{
					strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_TsiteDoorChk[2]);
					CTL_Lib.Alarm_Error_Occurrence(9280, dWARNING, strAlarmCode);
		...
		case 20150:
			...
			else
			{
				...
				{
					strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_TsiteDoorChk[2]);
					CTL_Lib.Alarm_Error_Occurrence(9290, dWARNING, strAlarmCode);
