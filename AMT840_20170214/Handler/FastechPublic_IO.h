...
//	int Check_IO_Port(int n_io, int n_time = 0, int n_flag = CTL_ON);
	...
	//james 2016.0810 센서없이 동작후 wait time 사용하는 것에 함수 사용
	void Set_IO_OnOff(int nIONo, int OnOff);//james 2016.0810 센서 없이 동작 시키는 실린더 등을 제어하기 위해 추가 
	int  Chk_IO_OnOff(int nIONo, int OnOff, int nWaitTime, int nLimitTime);//james 2016.0810 센서 없이 동작 시키는 실린더 등을 제어하기 위해 추가 
	int  Wait_IO_OnOff(int nIONo, int OnOff, int nWaitTime);//james 2016.0810 센서 없이 동작 시키는 실린더 등을 제어하기 위해 추가 
