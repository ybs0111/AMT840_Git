...
void CMyJamData::OnAlarmDisplay()
{
	...
	//st_alarm_info.strCode = _T("010006");
 	...
 	if (st_handler_info.nRunStatus == dJAM || st_handler_info.nRunStatus == dWARNING || st_handler_info.nRunStatus == dLOTEND)  
 	{
 		...
 		{
			sAlarmMsg.Format(_T("Jampos = %d, Alarm Code = %s"), st_alarm_info.nAlarmAssignSection, st_alarm_info.strCode);
 			...
 			else 
			{
 			...
 			if (st_alarm_info.strCode != "")
 			{
				...
				// 설정 시간이 없는 관계로 일단 주석처리한다. 
 				...
 				nChkTime = st_alarm_info.stl_cur_alarm_time - st_alarm_info.stl_prev_alarm_time; 
 				
 				if (nChkTime < st_basic_info.nAlarmDelayCnt )// st_alarm_info.nAlarmSaveDelayTM //3minute
 				{
...
//	char chMsg[1000];
	...
	// **************************************************************************
 	if (st_alarm_info.nCurState == 1)				
 	{
 		...
 		str_month_path = _T("c:\\AMT840\\Alarm\\Monthly\\MT") + strFileMonth + _T(".TXT");
 		str_day_path = _T("c:\\AMT840\\Alarm\\Daily\\DY") + strFileDay + _T(".TXT");
...
void CMyJamData::OnAlarmInfoLoad()
{
	...
	strLoadFile = _T("c:\\AMT840\\Bmp\\Error.txt"); 
