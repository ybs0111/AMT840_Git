...
#define INTERFACE_SERVER_MSG_NAME_4		"INTERFACE_SERVER_SHAREMEM_4"

CServerInterface4 clsInterS4;

CServerInterface4::CServerInterface4(void)
{
	// 공유 메모리 선언...../////////////////////////////////////////////////////////////////////////////////////////
	...
	m_hMemMapInter4	= CreateFileMapping(INVALID_HANDLE_VALUE,
										...
										PAGE_READWRITE,
										0,
										sizeof(INTER_SV_QUEUE_4),
										(LPCWSTR)INTERFACE_SERVER_MSG_NAME_4);
	if (m_hMemMapInter4 != NULL)
	{	
		if (GetLastError() != ERROR_ALREADY_EXISTS)
		{
			m_pQueueInter4 = (P_INTER_SV_QUEUE_4)MapViewOfFile(m_hMemMapInter4, FILE_MAP_ALL_ACCESS, 0, 0, 0);
...
CString	CServerInterface4::OnCharToString(char chData[], int nLength)
{
	CString strTmp;
	BSTR	buf;
	int		nCnt;
...
void CServerInterface4::OnMessageWrite(CString strMsg)
{
...
CString	CServerInterface4::OnMessageRead()
{
	CString strData = _T("");
...
void CServerInterface4::OnStringToChar(CString strData, char chData[])
{
	int nCnt;
	wchar_t *chBuf;
...
void CServerInterface4::OnDataReceive(CString strMsg)
{
	int nHead;
	int nLength;
	int nPos;
...
void CServerInterface4::OnDataAnalysis(CString strMsg)
{
	CString strFunc;
	CString strTemp;
	CString strData;
...
// 				}
		...
		st_server_info[INTERFACE_NETWORK_4].strSend = strRpy;
		::SendMessage(st_handler_info.hWnd, WM_SERVER_MSG + INTERFACE_NETWORK_4, SERVER_SEND, 0);

		st_server_info[INTERFACE_NETWORK_4].strSend = strData;
		::SendMessage(st_handler_info.hWnd, WM_SERVER_MSG + INTERFACE_NETWORK_4, SERVER_SEND, 0);
	}
							...
							if (strBarcode == st_test_site_info[nSite].st_pcb_info[i].strBarcode1D[0])
							{
								...
								else
								{
									...
									if(st_test_site_info[nSite].st_pcb_info[i].nFailCount > st_recipe_info.nFailCnt)
									{
										...
										//kwlee 2017.0105
										strTemp.Format(_T("TSite_4,Socket Off Slot : %d BIN : [%s]"),i + 1,strBin);
										//kwlee 2017.0918
	...
	else if (strFunc == _T("CHAMBER_DOOR"))
	{
		...
		if (strEqpID == st_test_site_info[nSite].strEqpID)
		{
			...
			if (st_handler_info.nAutoMode == YES)
			{
				...
				{
					clsTestSite4.m_nCloseDoor = YES;
					//kwlee 2016.1021 tester에서 프로그램 적용 되지 않아 원복함..
					st_handler_info.m_tlDooropen[3][0] = GetCurrentTime();
				}
				strData += _T("RESULT=PASS");
				strData += _T(" ");
				strData += _T("MSG=");
			...
			else
			{
				...
				{
					if (FAS_IO.get_in_bit(st_io_info.i_TsiteDoorChk[3], IO_OFF))
					{
						if (strChamDoor == _T("OPEN"))
						{
							clsTestSite4.m_nOpenDoor = YES;
...
CString CServerInterface4::OnDataItemAnalysis(int nOpt, int nPos, CString strMsg, CString strFind)
{
	CString strData = _T("");
	CString strHead;
	int length;
...
void CServerInterface4::OnCommunication()
{
	CString strMsg;
