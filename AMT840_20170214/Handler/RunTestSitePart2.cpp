...
CRunTestSitePart2 clsTestSite2;

CRunTestSitePart2::CRunTestSitePart2(void)
{
	m_nBdNum = 0;
	m_nTHD_ID = THD_TESTSITE_2;
	m_nIO_No  = 1;
	m_nRunStep = -1;
	// jtkim 20161005
	m_nInitStep = -1;
...
void CRunTestSitePart2::OnRunInit()
{
	int nRet_1, nRet_2=0;

	if (st_sync_info.nInit_Flag[THD_TESTSITE_2] != INIT_CLEAR)		return;		//INIT_CLEAR 일때만 초기화 작업을 한다. 초기화가 끝나면 INIT_CLEAR -> INIT_READY가 되기 떄문에...
	 st_sync_info.nInit_Flag[THD_TESTSITE_2] = INIT_READY; 
	...
	switch (m_nInitStep)
	{
		...
		case 100:
			nRet_1 = FAS_IO.get_in_bit(st_io_info.i_Tsite2ExistChk, IO_ON);
			...
			{
				m_strAlarmCode.Format(_T("8%d%04d"), IO_OFF, st_io_info.i_Tsite2ExistChk); 
				CTL_Lib.Alarm_Error_Occurrence(9100, dWARNING, m_strAlarmCode);
			}
			break;
		...
		case 1100:
			...
			{
				CTL_Lib.Alarm_Error_Occurrence(9110, dWARNING, m_strAlarmCode);
			}
			break;
		...
		case 1300:
			...
			else
			{
				...
				if (m_dwDoorWaitTime[2] > (DWORD) 60000)
				{
					...
					CTL_Lib.Alarm_Error_Occurrence(9120, dWARNING, m_strAlarmCode);
				}
			}
			break;
...
void CRunTestSitePart2::OnRunMove()
{
	...
	clsFunc.OnThreadFunctionStepTrace(15, m_nRunStep);	
	...
	{ 
		...
		case 2100:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9130, dWARNING, m_strAlarmCode);
		...
		case 3100:
			...
			else
			{
				...
				if (m_dwDoorWaitTime[2] > (DWORD) 60000)
				{
					...
					CTL_Lib.Alarm_Error_Occurrence(9140, dWARNING, m_strAlarmCode);
		...
		case 10200:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9150, dWARNING, m_strAlarmCode);
		...
		case 20200:
			...
			else
			{
				...
				{
					m_strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_ChamberOpenChk[m_nIO_No]); 
					CTL_Lib.Alarm_Error_Occurrence(9160, dWARNING, m_strAlarmCode);
...
void CRunTestSitePart2::OnThreadRun()
{
	//return;
	CTL_Lib.Process_TestChamber_AcMotor(0, THD_TESTSITE_2, 1); //chamber open/close AC Motor 제어하는 루틴	
	//OnCommunication();
	OnCommunication();
...
void CRunTestSitePart2::OnCommunication()
{
	COleDateTime	 tCurr;
	COleDateTimeSpan tDiff;

	int nSite = THD_TESTSITE_1 + 1;
	int	nTime;
	int i;
	...
	if (st_test_site_info[nSite].nTestBdStart == BD_START)
	{
		...
		if (st_basic_info.nModeDevice == WITH_DVC)
		{
			...
			{
				clsInterC2.OnTesterReady(st_test_site_info[nSite]);
...
// 				}
				...
				{ 
					...
					clsInterC2.OnModuleInfo(st_test_site_info[nSite]);
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_MODULE_OK)
				{
					...
					clsInterC2.OnTesterStart(st_test_site_info[nSite]);
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_PGM_CHK)
				{
					...
					if (nTime > 60000)
					{
						...
						clsInterC2.OnPgmDownload(st_test_site_info[nSite]);
					}
					else
					{
				...
				else if (st_test_site_info[nSite].nTestBdStart == BD_BIN_CHK)
				{
					...
					{
						clsInterC2.OnTesterAbort(st_test_site_info[nSite]);
...
// 							m_nCloseDoor = NO;
// 
// 							clsInterC2.OnChamberDoor(st_test_site_info[m_nTHD_ID], 1);
// 
// 							// jtkim 20160930 Log
// 							clsLog.OnLogTestSite(4, nSite, 0, st_test_site_info[nSite]);
// 						}
//  						if (m_nCloseDoor == YES)//2016.1031
// 						{
// 							st_handler_info.m_tlDooropen[1][1] = GetCurrentTime();
// 							st_handler_info.m_tlDooropen[1][2] = st_handler_info.m_tlDooropen[1][1] - st_handler_info.m_tlDooropen[1][0];
// 							if( st_handler_info.m_tlDooropen[1][2] <= 0 ) st_handler_info.m_tlDooropen[1][0] = GetCurrentTime();
// 							if( st_handler_info.m_tlDooropen[1][2] > 1500 )
// 							{
// 								m_nCloseDoor = NO;
// 
// 								clsInterC2.OnChamberDoor(st_test_site_info[m_nTHD_ID], 1);
...
void CRunTestSitePart2::Set_TestSite_UpDown( int nOnOff )
{
	...
	if(nOnOff == IO_OFF) //up
	{
		FAS_IO.set_out_bit( st_io_info.o_Tsite2DnCyl, IO_OFF);
		FAS_IO.set_out_bit( st_io_info.o_Tsite2UpCyl, IO_ON);
	}
	else //if(nOnOff == IO_ON)//down
	{
		FAS_IO.set_out_bit( st_io_info.o_Tsite2UpCyl, IO_OFF);
		FAS_IO.set_out_bit( st_io_info.o_Tsite2DnCyl, IO_ON);
	}

	if (nOnOff == IO_ON)
	{
		//LogFunction(_T("TEST_SITE"),_T("UP"),_T("TEST_SITE_2"),_T("CYLINDER"),1,strLogKey,strLogData);
		//2017.0109
		clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),0,_T("TEST_SITE_2"),_T("CYLINDER"),1,strLogKey,strLogData);
	}
	else
	{
		//LogFunction(_T("TEST_SITE"),_T("DOWN"),_T("TEST_SITE_2"),_T("CYLINDER"),1,strLogKey,strLogData);
		//2017.0109
		clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),0,_T("TEST_SITE_2"),_T("CYLINDER"),1,strLogKey,strLogData);
	}
}


int	CRunTestSitePart2::Chk_TestSite_UpDown( int nOnOff )
{
	CString strLogKey[10];
	CString	strLogData[10];
	...
	if (nOnOff == IO_OFF)//UP
	{
			...
			FAS_IO.get_in_bit(st_io_info.i_Tsite1DnChk, IO_OFF)	== IO_OFF)
		{
			...
			m_dwWaitTestSiteUpDown[0]	= GetCurrentTime();
				 ...
				 FAS_IO.get_in_bit(st_io_info.i_Tsite2UpChk, IO_ON)	== IO_ON &&
				 FAS_IO.get_in_bit(st_io_info.i_Tsite2DnChk, IO_OFF)	== IO_OFF)
		{
			...
			{
				//LogFunction(_T("TEST_SITE"),_T("UP"),_T("TEST_SITE_2"),_T("CYLINDER"),1,strLogKey,strLogData); //kwlee 2016.0812
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),1,_T("TEST_SITE_2"),_T("CYLINDER"),1,strLogKey,strLogData); 
				return RET_GOOD;
			}
		}
		else
		{
			...
			{
				m_strAlarmCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_Tsite2UpChk); 
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("UP"),1,_T("TEST_SITE_2"),_T("CYLINDER"),1,strLogKey,strLogData); 
				return RET_ERROR;
			}
	...
	{
		if (m_bTestSiteUpDownFlag == false &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite2UpChk, IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_Tsite2DnChk, IO_ON)	== IO_ON)
		{
			m_bTestSiteUpDownFlag		= true;
			m_dwWaitTestSiteUpDown[0]	= GetCurrentTime();
		}
			...
			FAS_IO.get_in_bit(st_io_info.i_Tsite2DnChk, IO_ON)	== IO_ON)
		{
			...
			if (m_dwWaitTestSiteUpDown[2] > (DWORD)st_wait_info.nOnWaitTime[nWaitTime])
			{
				//LogFunction(_T("TEST_SITE"),_T("DOWN"),_T("TEST_SITE_2"),_T("CYLINDER"),1,strLogKey,strLogData); //kwlee 2016.0812
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),1,_T("TEST_SITE_2"),_T("CYLINDER"),1,strLogKey,strLogData); 
				return RET_GOOD;
			}
		}
		else
		{
			...
			if (m_dwWaitTestSiteUpDown[2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				m_strAlarmCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_Tsite2DnChk); 
				//2017.0109
				clsLog.LogFunction(_T("TEST_SITE"),_T("DOWN"),1,_T("TEST_SITE_2"),_T("CYLINDER"),1,strLogKey,strLogData); 
				return RET_ERROR;
			}
...
void CRunTestSitePart2::OnRunDoor()
{
	int nRet_1, nRet_2;
	int nIO;
	...
	nIO = FAS_IO.get_in_bit(st_io_info.i_TsiteDoorChk[1], IO_OFF);
	...
	case 10150:
		...
		else
		{
			...
			if (m_dwDoorWaitTime[2] > (DWORD) 60000)
			{
				strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_TsiteDoorChk[1]);
				CTL_Lib.Alarm_Error_Occurrence(9170, dWARNING, strAlarmCode);
	...
	case 20150:
		...
		else
		{
			...
			{
				strAlarmCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_TsiteDoorChk[1]);
				CTL_Lib.Alarm_Error_Occurrence(9180, dWARNING, strAlarmCode);
