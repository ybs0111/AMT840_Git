...
CServerInterface3::CServerInterface3(void)
{
	// 공유 메모리 선언...../////////////////////////////////////////////////////////////////////////////////////////
	...
	m_hMemMapInter3	= CreateFileMapping(INVALID_HANDLE_VALUE,
										...
										PAGE_READWRITE,
										0,
										sizeof(INTER_SV_QUEUE_3),
										(LPCWSTR)INTERFACE_SERVER_MSG_NAME_3);
	if (m_hMemMapInter3 != NULL)
	{	
		if (GetLastError() != ERROR_ALREADY_EXISTS)
		{
			m_pQueueInter3 = (P_INTER_SV_QUEUE_3)MapViewOfFile(m_hMemMapInter3, FILE_MAP_ALL_ACCESS, 0, 0, 0);
...
CString	CServerInterface3::OnCharToString(char chData[], int nLength)
{
	CString strTmp;
	BSTR	buf;
	int		nCnt;
...
void CServerInterface3::OnMessageWrite(CString strMsg)
{
...
CString	CServerInterface3::OnMessageRead()
{
	CString strData = _T("");
...
void CServerInterface3::OnStringToChar(CString strData, char chData[])
{
	int nCnt;
	wchar_t *chBuf;
...
void CServerInterface3::OnDataReceive(CString strMsg)
{
	int nHead;
	int nLength;
	int nPos;
...
void CServerInterface3::OnDataAnalysis(CString strMsg)
{
	CString strFunc;
	CString strTemp;
	CString strData;
...
// 				}
		...
		st_server_info[INTERFACE_NETWORK_3].strSend = strRpy;
		::SendMessage(st_handler_info.hWnd, WM_SERVER_MSG + INTERFACE_NETWORK_3, SERVER_SEND, 0);

		st_server_info[INTERFACE_NETWORK_3].strSend = strData;
		::SendMessage(st_handler_info.hWnd, WM_SERVER_MSG + INTERFACE_NETWORK_3, SERVER_SEND, 0);
	}
	...
	else if (strFunc == _T("CHAMBER_DOOR"))
	{
		...
		if (strEqpID == st_test_site_info[nSite].strEqpID)
		{
			...
			if (st_handler_info.nAutoMode == YES)
			{
				...
				{
					clsTestSite3.m_nCloseDoor = YES;
					//kwlee 2016.1021 tester에서 프로그램 적용 되지 않아 원복함..
					st_handler_info.m_tlDooropen[2][0] = GetCurrentTime();
				}
				strData += _T("RESULT=PASS");
				strData += _T(" ");
				strData += _T("MSG=");
			...
			else
			{
				...
				{
					if (FAS_IO.get_in_bit(st_io_info.i_TsiteDoorChk[2], IO_OFF))
					{
						if (strChamDoor == _T("OPEN"))
						{
							clsTestSite3.m_nOpenDoor = YES;
...
CString CServerInterface3::OnDataItemAnalysis(int nOpt, int nPos, CString strMsg, CString strFind)
{
	CString strData = _T("");
	CString strHead;
	int length;
...
void CServerInterface3::OnCommunication()
{
	CString strMsg;
