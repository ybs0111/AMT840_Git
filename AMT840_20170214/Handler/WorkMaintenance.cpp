...
void CWorkMaintenance::OnDataApply()
{
	...
	//2017.0109
	if (st_lamp_info.nLampWaitTime			!= m_nLampFlickerTime[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		m_strLogData[0]	= _T("LAMP WAITTIME");
		m_strLogKey[1]	= _T("OLD");
		m_strLogData[1].Format(_T("%d"), st_lamp_info.nLampWaitTime);
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%d"), m_nLampFlickerTime[1]);

		clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 3, m_strLogKey, m_strLogData);
	}
	if (st_lamp_info.nBuzzerMode			!= m_nModeBuzzer[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		m_strLogData[0]	= _T("MODE BUZZER");
		m_strLogKey[1]	= _T("OLD");
		m_strLogData[1].Format(_T("%d"), st_lamp_info.nBuzzerMode);
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%d"), m_nModeBuzzer[1]);

		clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 3, m_strLogKey, m_strLogData);
	}
	if (st_lamp_info.nModeLampBlue			!= m_nModeLampBlue[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		m_strLogData[0]	= _T("LAMP BLUE");
		m_strLogKey[1]	= _T("OLD");
		m_strLogData[1].Format(_T("%d"), st_lamp_info.nModeLampBlue);
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%d"), m_nModeLampBlue[1]);

		clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 3, m_strLogKey, m_strLogData);
	}
	if (st_lamp_info.nModeLampBlueFlicker			!= m_nModeLampBlueFlicker[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		m_strLogData[0]	= _T("LAMP BLUE FLICKER");
		m_strLogKey[1]	= _T("OLD");
		m_strLogData[1].Format(_T("%d"), st_lamp_info.nModeLampBlueFlicker);
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%d"), m_nModeLampBlueFlicker[1]);

		clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 3, m_strLogKey, m_strLogData);
	}
	if (st_handler_info.nRunSpeed			!= m_nRunSpeed[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		m_strLogData[0]	= _T("RUN SPEED");
		m_strLogKey[1]	= _T("OLD");
		m_strLogData[1].Format(_T("%d"), st_handler_info.nRunSpeed);
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%d"), m_nRunSpeed[1]);

		clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 3, m_strLogKey, m_strLogData);
	}
	if (st_handler_info.nManualSpeed			!= m_nManualSpeed[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		m_strLogData[0]	= _T("MANUAL SPEED");
		m_strLogKey[1]	= _T("OLD");
		m_strLogData[1].Format(_T("%d"), st_handler_info.nManualSpeed);
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%d"), m_nManualSpeed[1]);

		clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 3, m_strLogKey, m_strLogData);
	}
	if (st_recipe_info.dRejTrayPick			!= m_dRejTrayPick[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		...
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%.3f"), m_dRejTrayPick[1]);

		clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 3, m_strLogKey, m_strLogData);
	}
	if (st_recipe_info.dRejTrayPlace			!= m_dRejTrayPlace[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		...
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%.3f"), m_dRejTrayPlace[1]);
	...
	{
		//2017.0109
		if (st_lamp_info.nLampR[i]			!= m_nRlamp[1][i])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("RED LAMP");
			m_strLogKey[1]	= _T("NUM");
			m_strLogData[1].Format(_T("%d"), i + 1);
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%d"), st_lamp_info.nLampR[i]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%d"), m_nRlamp[1][i]);

			clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_lamp_info.nLampY[i]			!= m_nYlamp[1][i])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("YELLOW LAMP");
			m_strLogKey[1]	= _T("NUM");
			m_strLogData[1].Format(_T("%d"), i + 1);
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%d"), st_lamp_info.nLampY[i]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%d"), m_nYlamp[1][i]);

			clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_lamp_info.nLampG[i]			!= m_nGlamp[1][i])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("GREEN LAMP");
			m_strLogKey[1]	= _T("NUM");
			m_strLogData[1].Format(_T("%d"), i + 1);
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%d"), st_lamp_info.nLampG[i]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%d"), m_nGlamp[1][i]);
	...
	if (st_basic_info.nModeXgem			!= m_nModeXgem[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		m_strLogData[0]	= _T("MODE XGEM");
		m_strLogKey[1]	= _T("OLD");
		m_strLogData[1].Format(_T("%d"), st_basic_info.nModeXgem);
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%d"), m_nModeXgem[1]);

		clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 3, m_strLogKey, m_strLogData);

		// jtkim 20161208
		::SendMessage(st_handler_info.hWnd, WM_WORK_COMMAND, XGEM_MODE, m_nModeXgem[1]);
	}
	if (st_basic_info.nModeXgemRunChk			!= m_nModeXgemRunChk[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		m_strLogData[0]	= _T("XGEM RUN");
		m_strLogKey[1]	= _T("OLD");
		m_strLogData[1].Format(_T("%d"), st_basic_info.nModeXgemRunChk);
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%d"), m_nModeXgemRunChk[1]);

		clsLog.LogConfig(_T("WORKMAINTENANCE"), _T("CHANGE"), st_basic_info.strDeviceName, 3, m_strLogKey, m_strLogData);
	}
	if (st_basic_info.nModeXgemInterface			!= m_nModeXgemInterface[1])
	{
		m_strLogKey[0]	= _T("TYPE");
		m_strLogData[0]	= _T("XGEM INTERFACE");
		m_strLogKey[1]	= _T("OLD");
		m_strLogData[1].Format(_T("%d"), st_basic_info.nModeXgemInterface);
		m_strLogKey[2]	= _T("NEW");
		m_strLogData[2].Format(_T("%d"), m_nModeXgemInterface[1]);
