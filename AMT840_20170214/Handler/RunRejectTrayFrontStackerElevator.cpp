// RejectTrayFrontStackerElevator.cpp : 구현 파일입니다.
...
// CRunRejectTrayFrontStackerElevator 멤버 함수
void CRunRejectTrayFrontStackerElevator::OnThreadRun(void)
{
	...
	CTL_Lib.Process_Tray_Conveyor_AcMotor(0, THD_REJECT_OUT_1_STACKER, st_io_info.o_Rej1UldCvyStkAcMotOnOff ); // elevator stacker AC Motor 제어하는 루틴	
...
void CRunRejectTrayFrontStackerElevator::OnRunInit(void)
{
	...
	if (st_sync_info.nInit_Flag[THD_REJECT_OUT_1_STACKER] != INIT_CLEAR)		return;		//INIT_CLEAR 일때만 초기화 작업을 한다. 초기화가 끝나면 INIT_CLEAR -> INIT_READY가 되기 떄문에...
	...
	switch (m_nInitStep)
	{
		...
		case 420:
			//kwlee 2016.0902
			st_sync_info.nConveyor_AcMotorCtl_Req[THD_REJECT_OUT_1_STACKER] = CTL_REQ; //cv rail ac motor 구동하여 적쨰된 트레이를 빼 낸다  
		...
		case 430:
			...
			nRet_2 = FAS_IO.get_in_bit(st_io_info.i_Rej1UldCvyTrayExistChk,	IO_OFF); //stacker tary 는 처음에는 없어야 한다   
	...
	{ 
		...
		case 0:
			if(st_lot_info[LOT_CURR].nLotStatus == LOT_START)
			{
				...
				if(st_lot_info[m_nLotProcessNum].nStacker_Reject_THD_Num == THD_REJECT_OUT_1_STACKER)//해당 스태커가 선택됐을때만 동작하자 
				{
		...
		case 310:
			nRet_1 = FAS_IO.get_in_bit(st_io_info.i_Rej1UldCvyTrayExistChk,	IO_OFF); //stacker tary 는 처음에는 없어야 한다  
			if(nRet_1 == IO_OFF)
			{
				m_nRunStep = 400;
			}
		...
		case 1000:
			//2017.0613
			nRet_1 = FAS_IO.get_in_bit(st_io_info.i_Rej1UldCvyTrayExistChk,	IO_OFF);
			if(nRet_1 == IO_ON)
			{
				//트레이  에러 
				m_strAlarmCode.Format(_T("8%d%04d"), IO_OFF, st_io_info.i_Rej1UldCvyTrayExistChk); 
				CTL_Lib.Alarm_Error_Occurrence(6990, dWARNING, m_strAlarmCode);
				break;
			}
			///////////////////////////////////////////////////////////////
		...
		case 1100:
			if(st_sync_info.nWorkRbt_Dvc_Req[THD_REJECT_OUT_1_STACKER][0] == CTL_REQ) //워크 로봇에서 uld 1 stacker에 트레이 공간을 요청 했다 
			{
				if(st_tray_info[THD_REJECT_OUT_1_STACKER].nTrayExist == CTL_YES) //트레이 정보가 있는 상태 
				{
					nRet_1 = COMI.Get_MotIOSensor(m_nAxisNum, MOT_SENS_SD); 	
					if(nRet_1 == BD_GOOD)
					{
			...
			{//james 2016.0916 
				...
				{				
					nRet_1 = FAS_IO.get_in_bit(st_io_info.i_Rej1StkTrayExistChk,	IO_ON); //elevator stacker tary 가 있어야 한다 
		...
		case 2000: 
			...
			{				
				...
				if(nRet_1 == BD_GOOD)
				{
					st_sync_info.nWorkRbt_Dvc_Req[THD_REJECT_OUT_1_STACKER][0] = CTL_READY; //현재 트레이의 작업이 가능하다 
					m_nRunStep = 1000;
				}
				else //if(nRet_1 == BD_ERROR) //플레이트에 트레이가 감지 안된 상태 
				{
		...
		case 3100:
			if(st_tray_info[THD_REJECT_OUT_1_STACKER].nTrayExist == CTL_YES) //트레이가 있으나, 작업할 공간이 없는 상태로 이미 한번 작업이 완료하느트레이이다. 트레이 정보가 있는 상태 
			{
				st_tray_info[THD_REJECT_OUT_1_STACKER].nTrayExist = CTL_NO; //tray 정보 클리어 
				...
				st_count_info.nUnLdStacker_Reject_TrayCnt[0]++; //unload good 1 stacker tray 적재 수량

				//if(st_count_info.nUnLdStacker_Reject_TrayCnt[0] >= st_basic_info.nUldGoodTrayStack_Count)//셋팅 수랴을 적재 했으면 배출한다  
				//kwlee 2017.0613
				if(st_count_info.nUnLdStacker_Reject_TrayCnt[0] >= st_basic_info.nRejectStackerTray_Cnt)//셋팅 수랴을 적재 했으면 배출한다  
				{
					m_nRunStep = 7000;
				}
				else
				...
				{
					clsXgem.OnMcTrayUnload(START,st_lot_info[m_nLotProcessNum].strLotNo,st_lot_info[m_nLotProcessNum].strPartNo,100 + st_count_info.nUnLdStacker_Reject_TrayCnt[0]);
				}
				///
			}
			else
		...
		case 3600:
			...
			if (st_basic_info.nModeXgem == YES)
			{
				clsXgem.OnMcTrayUnload(END,st_lot_info[m_nLotProcessNum].strLotNo,st_lot_info[m_nLotProcessNum].strPartNo,100 + st_count_info.nUnLdStacker_Reject_TrayCnt[0]);
			}
			///
			m_nRunStep = 1000;
			break;
		...
		case 7020:
			...
			{				
				...
				st_sync_info.nConveyor_AcMotorCtl_Req[THD_REJECT_OUT_1_STACKER] = CTL_FREE;//혹시 ac motor가 가동중이면 즉시 ㅈ정지 시킨다 
				m_nRunStep = 7100;
			}
			else
			{//트레이  에러 
