...
CComizoaPublic::CComizoaPublic()
{
	...
	mn_max_home_limit_time = 120000; //james 2016.1004 60000; //1분 
	mn_max_retry_cnt = 3;

	//2016.1129 모터문제가 없어지면 없앤다
	mn_errormsg_debug_mode = 1;
}
...
void CComizoaPublic::Set_MotorType_Init(int n_Axis, int nServoStep_Type) //james 2016.0819 추가 
{
	
	mn_type[n_Axis] = nServoStep_Type; //james 2016.0818 추가  Servo 또는 step motor 관련 셋팅 확인 건. (Inposition, home check 방법, comm처리 문제로 추가

	if( nServoStep_Type == MOT_SERVO ) // 2016.0819.ViboX
	{
		cmmCfgSetMioProperty( n_Axis, cmINP_EN, cmTRUE );
	}
	else
	{
		cmmCfgSetMioProperty( n_Axis, cmINP_EN, cmFALSE );
	}
}
...
//			sprintf(mc_normal_msg, "cmmLoadDll return error");
			...
			OnStringToChar(_T("cmmLoadDll return error"), mc_normal_msg);
...
//			sprintf(mc_normal_msg, "cmmGnDeviceLoad return error");
			...
			OnStringToChar(_T("cmmGnDeviceLoad return error"), mc_normal_msg);
			Debug_File_Create(0, mc_normal_msg);
		}
		...
		{	 
			if(n_OnOff == BD_YES) //james 2009.5.15 추가
			{
				...
				{//010008 E A "Motor position minus limit error(01 motor)."		
					strCode.Format(_T("%02d0008"), n_Axis);
					//wsprintfA(mc_alarmcode, "%S", strMsg);
					OnStringToChar(strCode, mc_alarmcode);
				...
				{//010009 E A "Motor position plus limit error(01 motor)."
					strCode.Format(_T("%02d0009"), n_Axis);
					//wsprintfA(mc_alarmcode, "%S", strCode);
					OnStringToChar(strCode, mc_alarmcode);
...
int CComizoaPublic::Get_Deviation_Position(int n_Axis, double d_AllowPos, double * d_Get_DevPos) 
{
	...
	{//허용오차보다 더 커지면 에러  
		...
		strCode.Format(_T("%02d0012"), n_Axis);
		//wsprintfA(mc_alarmcode, "%S", strMsg);
		OnStringToChar(strCode, mc_alarmcode);
...
int CComizoaPublic::Get_MotAlarmStatus(int n_Axis) 
{
	...
	{	//010004 E A "Motor Alarm Error(01 motor)."
		strCode.Format(_T("%02d0004"), n_Axis);
		//wsprintfA(mc_alarmcode, "%S", strMsg);
		OnStringToChar(strCode, mc_alarmcode);
...
double CComizoaPublic::Get_MotCurrentPos(int n_Axis) 
{
	...
	// jtkm 20130823
	if (0 && mn_type[n_Axis] == MOT_STEP) //20150428 james step 모터도 cme file 생성시 step motor로 설정하면 자동으로 cmCNT_FEED와 같이 읽어됨 
	{
...
//	CtlBdFunc.Send_Error_Message(0, n_Axis, "0", "0");
	...
	{//리턴 값이 에러일때 
		...
		strCode.Format(_T("%02d0013"), n_Axis);
		//wsprintfA(mc_alarmcode, "%S", strMsg);
		OnStringToChar(strCode, mc_alarmcode);
...
int CComizoaPublic::Start_SingleMove(int n_Axis, double d_Distance, int n_SpeedRatio) // (int ratio = 100) //2011.0105 변경 
{
	...
	//james 2016.0819 현재 위치와 목적위치가 0.01mm 값 이내이면 동작하지 말자
	nRet = Check_MotPosRange(n_Axis, d_Distance, 0.01);//md_allow_value[n_Axis]); //(int n_Axis, double d_Distance, int n_Range)  
	if(nRet == BD_GOOD)
	{
	...
	{
		if( n_Axis == M_LD_CV_LR_SHIFTER || n_Axis == M_LD_ELV || n_Axis == M_EMPTY_ELV || n_Axis == M_ULD_1_ELV || n_Axis == M_ULD_2_ELV )
		{
	...
	{
		if( n_Axis == M_RJ_EMPTY_ELV || n_Axis == M_RJ_ULD_1_ELV || n_Axis == M_RJ_ULD_2_ELV )
		{
			cmmSxStop(n_Axis, FALSE, FALSE); 
			return BD_RETRY;
		}
...
//		sprintf(Jamcode, "%02d0006", n_Axis); //on check error
		strCode.Format(_T("%02d0006"), n_Axis); 
		nCountMode	= 0;	
		nTypeMode	= eWARNING;
		nAlarmAssignSection = 0;
...
//			cmmSxStopEmg(n_Axis); //091012 동작중이면 정지 시키고 다시 동작한다 
			...
			//wsprintfA(mc_alarmcode, "%S", strMsg);
			OnStringToChar(strMsg, mc_alarmcode);
//			sprintf(mc_alarmcode, "%02d0013", n_Axis);  
			if(mn_errormsg_debug_mode)
			{
...
//		sprintf(Jamcode, "%02d0015", n_Axis); //on check error
		strCode.Format(_T("%02d0002"), n_Axis);
		nCountMode	= 0;	
		nTypeMode	= eWARNING;
		nAlarmAssignSection = 3;
...
//		sprintf(Jamcode, "%02d0015", n_Axis); //on check error
	...
	if (nRet == cmERR_NONE)
	{
		...
		//2017.0109
		clsLog.OnLogMotorMove(n_Axis, 1, d_Distance);
		return BD_GOOD;
	}
	else
	{
...
//		cmmSxStopEmg(n_Axis);  
	...
	if (nRetDone == cmERR_NONE)
	{
		...
		{	
			/////////////////////////////////////////////
			//james 2016.0819 추가. Inpotion 셋팅을 잘못 셋팅해도 서보모터이면, 무조건 확인하게 추가  //모터셋팅(cme 파일)에서 서보모터인데 Inposition 체크를 하지않아 동작완료 시그널 중 Inposition을 보지않아 위치에러등 문제발생
			if(mn_type[n_Axis] == MOT_SERVO) 
			{
			...
			if(nRet == BD_GOOD)
			{
				mn_retry_cnt[n_Axis] = 0; 
				//2017.0109
				clsLog.OnLogMotorMove(n_Axis, 0, d_Distance);
				return BD_GOOD;// 
			}
			else
			{
...
//	char Jamcode[10];
	...
	{
		nRet = Check_MotPosRange(mp_axisnum[n_MapIndex][i], dp_PosList[i], 0.01);
		if(nRet == BD_GOOD)
		{
			nCnt++;
		} 
	}
	if(nCnt == ml_axiscnt[n_MapIndex])
	{
		return BD_GOOD;// 
	}
	//////////////////////////////////////////////////////////////////
...
//			sprintf(Jamcode, "%02d0006", mp_axisnum[n_MapIndex][0]); //on check error
			strCode.Format(_T("%02d0006"), mp_axisnum[n_MapIndex][0]);
			nCountMode	= 0;	
			nTypeMode	= eWARNING;
			nAlarmAssignSection = 10;
...
//		sprintf(Jamcode, "%02d0015", mp_axisnum[n_MapIndex][0]); //on check error
		strCode.Format(_T("%02d0015"), mp_axisnum[n_MapIndex][0]);
		nCountMode	= 0;	
		nTypeMode	= eWARNING;
		nAlarmAssignSection = 11;
...
//			sprintf(Jamcode, "%02d0006", mp_axisnum[n_MapIndex][i]); //on check error
			//strCode.Format(_T("%02d0006"), mp_axisnum[n_MapIndex][i]);
			nCountMode	= 0;	
			nTypeMode	= eWARNING;
			nAlarmAssignSection = 12;
	...
	if (nRet == cmERR_NONE)
	{
		...
		for(i=0; i<ml_axiscnt[n_MapIndex]; i++)
		{
			if(mn_run_status == CTL_dRUN)	//090825 handler가 정상가동중일때만 커멘드 데이터를 백업, 메뉴얼 동작은 스킵 
			{
				...
				//2017.0109
				clsLog.OnLogMotorMove(mp_axisnum[n_MapIndex][i], 1, dp_PosList[i]);
			}
		}
	...
	if(nRet == cmERR_NONE) //함수 수행 성공 
	{
		...
		if(nListIsDone == cmTRUE) //0:리스트 모션이 동작중인 상태, 1: 모션이 완료된 상태 
		{
			...
			for(i=0; i<ml_axiscnt[n_MapIndex]; i++) //최대 한번에 4축 보간동작으로 이동가능 
			{
				...
				/////////////////////////////////////////////
				//james 2016.0819 추가. Inpotion 셋팅을 잘못 셋팅해도 서보모터이면, 무조건 확인하게 추가  //모터셋팅(cme 파일)에서 서보모터인데 Inposition 체크를 하지않아 동작완료 시그널 중 Inposition을 보지않아 위치에러등 문제발생
				if(mn_type[mp_axisnum[n_MapIndex][i]] == MOT_SERVO) 
				{
					nRet = Get_MotInposition(mp_axisnum[n_MapIndex][i]);
					if(nRet != BD_GOOD)
					{
						return BD_PROCEED;
					}
			...
			if(nCurrPosEndCnt >= ml_axiscnt[n_MapIndex]) //james test
			{
				...
				mn_linear_retry_cnt[n_MapIndex] = 0;  
				//2017.0109
				clsLog.OnLogMotorMove(mp_axisnum[n_MapIndex][i], 0, dp_PosList[i]);
				return BD_GOOD;
			}
			else
			{//보간동작 완료했으나 위치가 문제인 상태 
				...
				if (ml_linear_move_time[n_MapIndex][2] > 2000) //090609 MOT_MOVE_LIMITTIME)
				{
					if(mn_linear_retry_cnt[n_MapIndex] > mn_max_retry_cnt)
					{
						...
						strCode.Format(_T("%02d0006"), mp_axisnum[n_MapIndex][nError]);
						//wsprintfA(mc_alarmcode, "%S", strMsg);
						OnStringToChar(strCode, mc_alarmcode);
//						sprintf(mc_alarmcode, "%02d0015", n_MapIndex);
						if(mn_errormsg_debug_mode)
						{
							...
							//wsprintfA(mc_normal_msg, "%S", strMsg);
							OnStringToChar(strCode, mc_normal_msg);
...
//	int    nSafetyCheck = 0, Ret, i=0;
	...
	{
		nRet = Check_MotPosRange(lp_AxisNum[i], dp_PosList[i], 0.01);
		if(nRet == BD_GOOD)
		{
			nCnt++;
		} 
...
//			cmmSxStopEmg(lp_AxisNum[i]);
	...
	for(i=0; i<l_AxisCnt; i++)
	{
		...
		if(nRet == BD_ERROR)
		{//010002 E A "Motor Move Setting Error01 motor)."
			strCode.Format(_T("%02d0002"), lp_AxisNum[i]);
			//wsprintfA(mc_alarmcode, "%S", strMsg);
			OnStringToChar(strCode, mc_alarmcode);
...
//				sprintf(mc_normal_msg, "[axis=%d] multi move speed return error", lp_AxisNum[i]);
	...
	{
		strCode.Format(_T("%02d0002"), lp_AxisNum[0]);
		//wsprintfA(mc_alarmcode, "%S", strMsg);
		OnStringToChar(strCode, mc_alarmcode);
	...
	if(nRet == cmERR_NONE) //수행 성공 
	{
		if(nListIsDone == cmTRUE) //0:리스트 모션이 동작중인 상태, 1: 모션이 완료된 상태 
		{
			...
			for(i=0; i<l_AxisCnt; i++) //최대 한버에 4축 보간동작으로 이동가능 
			{
				...
				/////////////////////////////////////////////
				//james 2016.0819 추가. Inpotion 셋팅을 잘못 셋팅해도 서보모터이면, 무조건 확인하게 추가  //모터셋팅(cme 파일)에서 서보모터인데 Inposition 체크를 하지않아 동작완료 시그널 중 Inposition을 보지않아 위치에러등 문제발생
				if(mn_type[lp_AxisNum[i]] == MOT_SERVO) 
				{
					nRet = Get_MotInposition(lp_AxisNum[i]);
					if(nRet != BD_GOOD)
					{
						return BD_PROCEED;
					}
			...
			{//보간동작 완료 
				if (ml_motion_move_time[lp_AxisNum[0]][2] > 2000) //090609 MOT_MOVE_LIMITTIME)
				{
					if(mn_retry_cnt[lp_AxisNum[0]] > mn_max_retry_cnt)
					{
						...
						strCode.Format(_T("%02d0006"), lp_AxisNum[0]);
						//wsprintfA(mc_alarmcode, "%S", strMsg);
						OnStringToChar(strCode, mc_alarmcode);
...
//Index Map 번호 및 셋팅 
	...
	{//Index Map 작성 에러 
		...
		//010002 E A "Motor Move Setting Error01 motor)."
		strCode.Format(_T("%02d0002"), ml_coord_totalaxisnum[n_LmIndexNum][0]);
		//wsprintfA(mc_alarmcode, "%S", strMsg);
		OnStringToChar(strCode, mc_alarmcode);
	...
	for(i=0; i<ml_coord_totalaxiscnt[n_LmIndexNum]; i++) //단축이든 보간이든 한번 동작을 1회로 보아 이동하는 횟수를 지정한다  
	{
		...
		{	
			...
			if (nRet != BD_YES)//cmERR_NONE)
			{
				//cmmLmAbortMotion(n_LmIndexNum); //동작중 에러 발생시 강제 종료 시킨다 
				//010002 E A "Motor Move Setting Error01 motor)."
				strCode.Format(_T("%02d0002"), ml_coord_subaxisnum[n_LmIndexNum][i][0]);
				//wsprintfA(mc_alarmcode, "%S", strMsg);
				OnStringToChar(strCode, mc_alarmcode);
...
//				sprintf(mc_normal_msg, "[axis=%d] coord Start_LinearMove return_0 error",ml_coord_subaxisnum[n_LmIndexNum][i][0]);
	...
	if (nRet != cmERR_NONE)
	{
		...
		//010002 E A "Motor Move Setting Error01 motor)."
		strCode.Format(_T("%02d0002"), ml_coord_subaxisnum[n_LmIndexNum][0][0]);
		//wsprintfA(mc_alarmcode, "%S", strMsg);
		OnStringToChar(strCode, mc_alarmcode);
		...
		if(lListIsDone == cmTRUE) //BD_YES) //0:리스트 모션이 동작중인 상태, 1: 모션이 완료된 상태 
		{
			...
			{//보간동작 완료 
				if(ml_motion_move_time[ml_coord_subaxisnum[n_LmIndexNum][0][0]][2] > 2000) //090609 MOT_MOVE_LIMITTIME)
				{
					...
					{
						//010006 E A "Motor Move Check Error(01 motor)."
						strCode.Format(_T("%02d0006"), ml_coord_subaxisnum[n_LmIndexNum][0][0]);
						//wsprintfA(mc_alarmcode, "%S", strMsg);
						OnStringToChar(strCode, mc_alarmcode);
		...
		case 210 :
			...
			else
			{
				if (mn_retry_cnt[n_Axis] > mn_max_retry_cnt)
				{
					...
					strCode.Format(_T("%02d0005"), n_Axis);
					//wsprintfA(mc_alarmcode, "%S", strMsg);
					OnStringToChar(strCode, mc_alarmcode);
		...
		case 300 :  // 모터 HOMING 
			...
			else
			{
				...
				{				
					...
					strCode.Format(_T("%02d0003"), n_Axis);
					//wsprintfA(mc_alarmcode, "%S", strMsg);
					OnStringToChar(strCode, mc_alarmcode);
...
*/
	...
	{
		//010002 E A "Motor Move Setting Error01 motor)."
		strCode.Format(_T("%02d0002"), l_AxisNum);
		//wsprintfA(mc_alarmcode, "%S", strMsg);
		OnStringToChar(strCode, mc_alarmcode);
	...
	if(!fp)
 	{
		...
		OnStringToChar(_T("File open failure!.._4"), mc_normal_msg);
...
//	_snprintf(fileName, sizeof(fileName), "%S", mstr_content);
	...
	OnStringToChar(mstr_content, fileName);
//	fprintf(fp,_T("%S\n"),mstr_content) ;
	fprintf(fp,fileName) ;
	...
	if (ferror(fp))  
	{
		...
		OnStringToChar(_T("Debug_File_Create ferror return error"), mc_normal_msg);
...
void CComizoaPublic::OnStringToChar(CString strData, char chData[])
{
	int nCnt;
	wchar_t *chBuf;
