...
#define INTERFACE_SERVER_MSG_NAME_2		"INTERFACE_SERVER_SHAREMEM_2"

CServerInterface2 clsInterS2;

CServerInterface2::CServerInterface2(void)
{
	// 공유 메모리 선언...../////////////////////////////////////////////////////////////////////////////////////////
	...
	m_hMemMapInter2	= CreateFileMapping(INVALID_HANDLE_VALUE,
										...
										PAGE_READWRITE,
										0,
										sizeof(INTER_SV_QUEUE_2),
										(LPCWSTR)INTERFACE_SERVER_MSG_NAME_2);
	if (m_hMemMapInter2 != NULL)
	{	
		if (GetLastError() != ERROR_ALREADY_EXISTS)
		{
			m_pQueueInter2 = (P_INTER_SV_QUEUE_2)MapViewOfFile(m_hMemMapInter2, FILE_MAP_ALL_ACCESS, 0, 0, 0);
...
CString	CServerInterface2::OnCharToString(char chData[], int nLength)
{
	CString strTmp;
	BSTR	buf;
	int		nCnt;
...
void CServerInterface2::OnMessageWrite(CString strMsg)
{
...
CString	CServerInterface2::OnMessageRead()
{
	CString strData = _T("");
...
void CServerInterface2::OnStringToChar(CString strData, char chData[])
{
	int nCnt;
	wchar_t *chBuf;
...
void CServerInterface2::OnDataReceive(CString strMsg)
{
	int nHead;
	int nLength;
	int nPos;
...
void CServerInterface2::OnDataAnalysis(CString strMsg)
{
	CString strFunc;
	CString strTemp;
	CString strData;
...
// 				}
		...
		st_server_info[INTERFACE_NETWORK_2].strSend = strRpy;
		::SendMessage(st_handler_info.hWnd, WM_SERVER_MSG + INTERFACE_NETWORK_2, SERVER_SEND, 0);

		st_server_info[INTERFACE_NETWORK_2].strSend = strData;
		::SendMessage(st_handler_info.hWnd, WM_SERVER_MSG + INTERFACE_NETWORK_2, SERVER_SEND, 0);
	}
							...
							if (strBarcode == st_test_site_info[nSite].st_pcb_info[i].strBarcode1D[0])
							{
								...
								else
								{
									...
									if(st_test_site_info[nSite].st_pcb_info[i].nFailCount > st_recipe_info.nFailCnt)
									{
										...
										//kwlee 2017.0105
										strTemp.Format(_T("TSite_2,Socket Off Slot : %d BIN : [%s]"),i + 1,strBin);
										//kwlee 2017.0918
	...
	else if (strFunc == _T("CHAMBER_DOOR"))
	{
		...
		if (strEqpID == st_test_site_info[nSite].strEqpID)
		{
			...
			if (st_handler_info.nAutoMode == YES)
			{
				...
				{
					clsTestSite2.m_nCloseDoor = YES;
					//kwlee 2016.1021 tester에서 프로그램 적용 되지 않아 원복함..
					st_handler_info.m_tlDooropen[1][0] = GetCurrentTime();
				}
				strData += _T("RESULT=PASS");
				strData += _T(" ");
				strData += _T("MSG=");
			...
			else
			{
				...
				{
					if (FAS_IO.get_in_bit(st_io_info.i_TsiteDoorChk[1], IO_OFF))
					{
						if (strChamDoor == _T("OPEN"))
						{
							clsTestSite2.m_nOpenDoor = YES;
...
CString CServerInterface2::OnDataItemAnalysis(int nOpt, int nPos, CString strMsg, CString strFind)
{
	CString strData = _T("");
	CString strHead;
	int length;
...
void CServerInterface2::OnCommunication()
{
	CString strMsg;
