...
void CXgemClient::OnXgemRecipe(int nMode, CString strPath, CString strModel)
{
	...
	if (nMode == 0)
	{
		...
		strTemp.Format(_T("%d"), st_basic_info.nMode1DBcr);
		:: WritePrivateProfileString(_T("Mode"), _T("BARCODE_1D_USE"), LPCTSTR(strTemp), strRecipePath);

		strTemp.Format(_T("%d"), st_basic_info.nMode2DBcr);
		:: WritePrivateProfileString(_T("Mode"), _T("BARCODE_2D_USE"), LPCTSTR(strTemp), strRecipePath);
	}
	else
	{
		strRecipePath = strPath + strModel;
