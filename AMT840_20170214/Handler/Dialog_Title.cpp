...
void CDialog_Title::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_MSG_BARCODE_1D,			m_msgBarcode_1D);
	DDX_Control(pDX, IDC_MSG_BARCODE_2D,			m_msgBarcode_2D);
...
void CDialog_Title::OnInit_Label()
{
	...
	m_msgBarcode_1D.SetFont(clsFunc.m_pFont[1]);
	m_msgBarcode_1D.SetCenterText();
	m_msgBarcode_1D.SetColor(RGB(0,0,255));
	m_msgBarcode_1D.SetGradientColor(RGB(0,0,0));
	m_msgBarcode_1D.SetTextColor(RGB(255,255,255));

	m_msgBarcode_2D.SetFont(clsFunc.m_pFont[1]);
	m_msgBarcode_2D.SetCenterText();
	m_msgBarcode_2D.SetColor(RGB(0,0,255));
	m_msgBarcode_2D.SetGradientColor(RGB(0,0,0));
	m_msgBarcode_2D.SetTextColor(RGB(255,255,255));
...
LRESULT CDialog_Title::OnStatusDataUpdate(WPARAM wParam,LPARAM lParam)
{
	...
	//kwlee 2016.0825
	else if (wParam == BCR_1D)	// Device Mode
	{
		OnTitle1D_BcrDisplay((int)lParam);
	}
	else if (wParam == BCR_2D)	// Device Mode
	{
		OnTitle2D_BcrDisplay((int)lParam);
	}
	return 0;
}
...
void CDialog_Title::OnTitleFormNameChange(int nViewID)
{
	...
	switch(nViewID)
	{
		...
		case IDW_SCREEN_SET_RECIPE:
			...
			//kwlee 2016.0913
		case IDW_SCREEN_VISION_TEACH:
			strFormName += _T("\n");
			strFormName += _T(" Screen: Vision Teach");
			break;
			///
...
//kwlee 2016.0825
void CDialog_Title::OnTitle1D_BcrDisplay(int n1D_Bcr)
{
	CString strBcr;   // 임시 저장 변수

	switch(n1D_Bcr)
	{
					
	case BCR_NOT_USE:
		m_msgBarcode_1D.SetColor(RGB(255,0,0));
		strBcr = _T("1D NOT USE");
	...
	m_msgBarcode_1D.SetWindowText(strBcr);
}

void CDialog_Title::OnTitle2D_BcrDisplay(int n2D_Bcr)
{
	CString strBcr;   // 임시 저장 변수
	...
	{
	case BCR_NOT_USE:
		m_msgBarcode_2D.SetColor(RGB(255,0,0));
		strBcr = _T("2D NOT USE");
		break;
	...
	m_msgBarcode_2D.SetWindowText(strBcr);
}