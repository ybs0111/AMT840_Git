...
#include "Variable.h"
typedef struct _INTER_CLIENT_QUEUE_2_
{
	int			m_nRear;
	int			m_nFront;
	int			m_nCount;
...
} INTER_CLIENT_QUEUE_2, *P_INTER_CLIENT_QUEUE_2;

class CClientInterface2
{
public:
	int							m_nRetry;
	int							m_nCommStep;
	...
	P_INTER_CLIENT_QUEUE_2		m_pQueueInter2;
...
public:
	CClientInterface2(void);
	~CClientInterface2(void);
};

extern CClientInterface2 clsInterC2;
