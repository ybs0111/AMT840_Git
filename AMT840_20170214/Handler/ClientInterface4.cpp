...
#define INTERFACE_CLIENT_MSG_NAME_4		"INTERFACE_CLIENT_SHAREMEM_4"

CClientInterface4 clsInterC4;

CClientInterface4::CClientInterface4(void)
{
	// 공유 메모리 선언...../////////////////////////////////////////////////////////////////////////////////////////
	m_hMemMapInter4	= NULL;
	m_pQueueInter4	= NULL;

	m_hMemMapInter4 = CreateFileMapping(INVALID_HANDLE_VALUE,
										 NULL,
										 PAGE_READWRITE,
										 0,
										 sizeof(INTER_CLIENT_QUEUE_4),
										 (LPCWSTR)INTERFACE_CLIENT_MSG_NAME_4);
	if (m_hMemMapInter4 != NULL)
	{	
		if (GetLastError() != ERROR_ALREADY_EXISTS)
		{	
			m_pQueueInter4 = (P_INTER_CLIENT_QUEUE_4)MapViewOfFile(m_hMemMapInter4, FILE_MAP_ALL_ACCESS, 0, 0, 0);
			if (m_pQueueInter4 != NULL)
			{
				m_pQueueInter4->m_nFront	= 0;
				m_pQueueInter4->m_nRear	= 0;
				m_pQueueInter4->m_nCount	= 0;
			}
		}
	}
...
CClientInterface4::~CClientInterface4(void)
{
	if (m_pQueueInter4 != NULL)	UnmapViewOfFile(m_pQueueInter4);	
	if (m_hMemMapInter4 != NULL)	CloseHandle(m_hMemMapInter4);
}


void CClientInterface4::OnCommunication()
{
	...
	switch (m_nCommStep)
	{
		case 0:
			// 공유메모리에 전송할 데이터가 있으면 
			if (m_pQueueInter4->m_nCount <= 0) break;
		...
		case 2000:
			...
			else
			{
				...
				::SendMessage(st_handler_info.hWnd, WM_CLIENT_MSG + INTERFACE_NETWORK_4, CLIENT_CONNECT, 0);
			...
			else
			{
				...
				if (m_dwWaitTime[2] > 30000)
				{
					...
					if (m_nRetry > 1)
					{
						...
						::SendMessage(st_handler_info.hWnd, WM_CLIENT_MSG + INTERFACE_NETWORK_4, CLIENT_CLOSE, 0);

						m_dwWaitTime[0] = GetTickCount();
						CTL_Lib.Alarm_Error_Occurrence(20000, dWARNING, _T("740000"));
		...
		case 2200:
			m_dwWaitTime[0] = GetTickCount();
			st_client_info[INTERFACE_NETWORK_4].strSend = m_strSendMsg;

			::SendMessage(st_handler_info.hWnd, WM_CLIENT_MSG + INTERFACE_NETWORK_4, CLIENT_SEND, 0);
		...
		case 2300:
			if (m_nCommBin == BD_DATA_GOOD)
			{
				...
				///////////////////////////////////////////////
				//2016.1203
				st_product_info.dTestTime += (double)st_test_site_info[THD_TESTSITE_4].dwBdTime;
			...
			else if (m_nCommBin == BD_DATA_REJECT)
			{
				...
				//2016.1012
				strErr.Format( _T("Tester4 : Data reject. Test check please" ) );					
			...
			else
			{
				...
				if (m_dwWaitTime[2] > 300000)//최대 3분을 기다리다가 다시 명령어 전송 //2016.1027
				{
					...
					if (m_nRetry > 1)
					{
						...
						m_dwWaitTime[0] = GetTickCount();
						CTL_Lib.Alarm_Error_Occurrence(20001, dWARNING, _T("740001"));
...
CString	CClientInterface4::OnCharToString(char chData[], int nLength)
{
	CString strTmp;
	BSTR	buf;
	int		nCnt;
...
void CClientInterface4::OnStringToChar(CString strData, char chData[])
{
	int nCnt;
	wchar_t *chBuf;
...
void CClientInterface4::OnMessageWrite(CString strMsg)
{
	CString strLog = _T("[HANDLER >> EC SERVER] ");

	if (m_hMemMapInter4 != NULL && m_pQueueInter4 != NULL)
	{
		strLog += strMsg;
		clsFunc.OnLogFileAdd(4, strLog);

		OnStringToChar(strMsg, m_pQueueInter4->m_chMsg[m_pQueueInter4->m_nRear]);

		m_pQueueInter4->m_nRear++;
		
		if (m_pQueueInter4->m_nRear >= 10) m_pQueueInter4->m_nRear = 0;
		
		m_pQueueInter4->m_nCount++;
	}
}

CString	CClientInterface4::OnMessageRead()
{
	CString strData = _T("");
	
	if (m_hMemMapInter4 != NULL && m_pQueueInter4 != NULL)
	{
		if (m_pQueueInter4->m_nCount > 0 )
		{
			strData = m_pQueueInter4->m_chMsg[m_pQueueInter4->m_nFront];
			
			m_pQueueInter4->m_nFront++;
			if (m_pQueueInter4->m_nFront >= 10) m_pQueueInter4->m_nFront = 0;
...
void CClientInterface4::OnDataReceive(CString strMsg)
{
	int nHead;
	int nLength;
	int nPos;
	...
	CString strData;
	CString strFunc;
	CString strLog = _T("[INTERFACE SERVER 4 >> HANDLER] ");
	...
	{
		strTemp.Format(_T("[INTERFACE_CLIENT_4_NETWORK] %s"), strMsg);
		clsMem.OnNormalMessageWrite(strTemp);
	}
}


void CClientInterface4::OnDataAnalysis(CString strMsg)
{
	CString strFunc;
	CString strData;
	CString strTemp;
...
// 				}
		...
		else
		{
			...
			//2016.1101				
			st_handler_info.mstr_event_msg[0].Format(_T("%d site %s,엔지니어 호출 해주세요"), 4, m_strErrorMsg );
	...
	else if (strFunc == _T("TEST_START"))
	{
		...
		if (strData == _T("PASS"))
		{
			...
			for (i=0; i<8; i++)
			{
				...
				{
					strLogData[0]	= st_test_site_info[THD_TESTSITE_4].st_pcb_info[i].strSerialNo;
					clsLog.LogProcess(_T("TESTER#4"), 
 								  _T("CDIMM TEST"), 
 								  0, 
 								  st_test_site_info[THD_TESTSITE_4].st_pcb_info[i].strSerialNo, 
 								  st_test_site_info[THD_TESTSITE_4].st_pcb_info[i].strLotNo, 
...
CString CClientInterface4::OnDataItemAnalysis(int nOpt, int nPos, CString strMsg, CString strFind)
{
	CString strData = _T("");
	CString strHead;
	int length;
...
void CClientInterface4::OnTesterReady(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
	...
	/*test_site.strOptCode.Format(_T("00"));*/
...
void CClientInterface4::OnSocketInfo(tagTEST_SITE_INFO &test_site)
{
	int i;
...
void CClientInterface4::OnModuleInfo(tagTEST_SITE_INFO &test_site)
{
	int i;
...
void CClientInterface4::OnTesterStart(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface4::OnPgmDownload(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface4::OnLotComplete(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface4::OnTesterOption(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface4::OnTesterAbort(tagTEST_SITE_INFO &test_site)
{
	CString strTemp;
	CString strData;
	CString strMsg;
...
void CClientInterface4::OnChamberDoor(tagTEST_SITE_INFO &test_site, int nStatus)
{
	CString strTemp;
	CString strData;
	CString strMsg;
