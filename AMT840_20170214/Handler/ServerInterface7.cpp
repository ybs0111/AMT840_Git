...
#define INTERFACE_SERVER_MSG_NAME_7		"INTERFACE_SERVER_SHAREMEM_7"

CServerInterface7 clsInterS7;

CServerInterface7::CServerInterface7(void)
{
	// 공유 메모리 선언...../////////////////////////////////////////////////////////////////////////////////////////
	...
	m_hMemMapInter7	= CreateFileMapping(INVALID_HANDLE_VALUE,
										...
										PAGE_READWRITE,
										0,
										sizeof(INTER_SV_QUEUE_7),
										(LPCWSTR)INTERFACE_SERVER_MSG_NAME_7);
	if (m_hMemMapInter7 != NULL)
	{	
		if (GetLastError() != ERROR_ALREADY_EXISTS)
		{
			m_pQueueInter7 = (P_INTER_SV_QUEUE_7)MapViewOfFile(m_hMemMapInter7, FILE_MAP_ALL_ACCESS, 0, 0, 0);
...
CString	CServerInterface7::OnCharToString(char chData[], int nLength)
{
	CString strTmp;
	BSTR	buf;
	int		nCnt;
...
void CServerInterface7::OnMessageWrite(CString strMsg)
{
...
CString	CServerInterface7::OnMessageRead()
{
	CString strData = _T("");
...
void CServerInterface7::OnStringToChar(CString strData, char chData[])
{
	int nCnt;
	wchar_t *chBuf;
...
void CServerInterface7::OnDataReceive(CString strMsg)
{
	int nHead;
	int nLength;
	int nPos;
...
void CServerInterface7::OnDataAnalysis(CString strMsg)
{
	CString strFunc;
	CString strTemp;
	CString strData;
...
// 				}
		...
		st_server_info[INTERFACE_NETWORK_7].strSend = strRpy;
		::SendMessage(st_handler_info.hWnd, WM_SERVER_MSG + INTERFACE_NETWORK_7, SERVER_SEND, 0);

		st_server_info[INTERFACE_NETWORK_7].strSend = strData;
		::SendMessage(st_handler_info.hWnd, WM_SERVER_MSG + INTERFACE_NETWORK_7, SERVER_SEND, 0);
	}
							...
							if (strBarcode == st_test_site_info[nSite].st_pcb_info[i].strBarcode1D[0])
							{
								...
								else
								{
									...
									if(st_test_site_info[nSite].st_pcb_info[i].nFailCount > st_recipe_info.nFailCnt)
									{
										...
										//kwlee 2017.0105
										strTemp.Format(_T("TSite_7,Socket Off Slot : %d BIN : [%s]"),i + 1,strBin);
										//kwlee 2017.0918
	...
	else if (strFunc == _T("CHAMBER_DOOR"))
	{
		...
		if (strEqpID == st_test_site_info[nSite].strEqpID)
		{
			...
			if (st_handler_info.nAutoMode == YES)
			{
				...
				{
					clsTestSite7.m_nCloseDoor = YES;
					//kwlee 2016.1021 tester에서 프로그램 적용 되지 않아 원복함..
					st_handler_info.m_tlDooropen[6][0] = GetCurrentTime();
				}
				strData += _T("RESULT=PASS");
				strData += _T(" ");
				strData += _T("MSG=");
			...
			else
			{
				...
				{
					if (FAS_IO.get_in_bit(st_io_info.i_TsiteDoorChk[6], IO_OFF))
					{
						if (strChamDoor == _T("OPEN"))
						{
							clsTestSite7.m_nOpenDoor = YES;
...
CString CServerInterface7::OnDataItemAnalysis(int nOpt, int nPos, CString strMsg, CString strFind)
{
	CString strData = _T("");
	CString strHead;
	int length;
...
void CServerInterface7::OnCommunication()
{
	CString strMsg;
