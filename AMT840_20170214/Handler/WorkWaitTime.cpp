...
void CWorkWaitTime::OnDataApply()
{
	...
	{
		//2017.0109
		if (st_wait_info.nOnWaitTime[i]				!= m_nOnWaitTime[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("ON TIME");
			m_strLogKey[1]	= _T("NAME");
			m_strLogData[1].Format(_T("%s"), m_strWaitTimeLabel[i]);
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%d"), st_wait_info.nOnWaitTime[i]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%d"), m_nOnWaitTime[i][1]);

			clsLog.LogConfig(_T("WORKWAITTIME"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_wait_info.nOffWaitTime[i]				!= m_nOffWaitTime[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("ON TIME");
			m_strLogKey[1]	= _T("NAME");
			m_strLogData[1].Format(_T("%s"), m_strWaitTimeLabel[i]);
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%d"), st_wait_info.nOffWaitTime[i]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%d"), m_nOffWaitTime[i][1]);

			clsLog.LogConfig(_T("WORKWAITTIME"), _T("CHANGE"), st_basic_info.strDeviceName, 4, m_strLogKey, m_strLogData);
		}
		if (st_wait_info.nLimitWaitTime[i]				!= m_nLimitWaitTime[i][1])
		{
			m_strLogKey[0]	= _T("TYPE");
			m_strLogData[0]	= _T("LIMIT TIME");
			m_strLogKey[1]	= _T("NAME");
			m_strLogData[1].Format(_T("%s"), m_strWaitTimeLabel[i]);
			m_strLogKey[2]	= _T("OLD");
			m_strLogData[2].Format(_T("%d"), st_wait_info.nLimitWaitTime[i]);
			m_strLogKey[3]	= _T("NEW");
			m_strLogData[3].Format(_T("%d"), m_nLimitWaitTime[i][1]);
